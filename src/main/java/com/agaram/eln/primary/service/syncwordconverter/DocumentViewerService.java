...
public class DocumentViewerService {
    ...
    private void handleExistingTemplate(Reports data, byte[] documentBytes, Response response) throws Exception {
        Reports existingReport = reportsRepository.findById(data.getReportcode()).orElse(null);
...
//        }
    ...
    private void saveToGridFSonReportVersion(ReportsVersion data, byte[] documentBytes) throws IOException {
        try {        	
        	...
        	GridFSFile file = gridFsTemplate.findOne(new Query(Criteria.where("filename").is(reportName)));
    	    if (file != null) {
    	        gridFsTemplate.delete(new Query(Criteria.where("filename").is(reportName)));
    	    }
    	    
    	    DBObject meta = new BasicDBObject();
    	    meta.put("encoding", "UTF-16");
    	    
    	    gridFsTemplate.store(new ByteArrayInputStream(documentBytes), reportName, meta);
	...
	private String retrieveJsonContentFromGridFS(Reports objReport) throws IOException {
 		...
 		jsonContent = new BufferedReader(new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8))
	...
	public Reports approveReport(Reports objReport) {
		...
		Optional<Reports> optionalReport = Optional.ofNullable(reportsRepository.findById(objReport.getReportcode())).orElse(null);
