...
//	List<ParserTechnique> getParserTechniqueByMethodKey(final int methodKey);
	...
	@Transactional
	@Query(value = "SELECT ParserTechnique.* FROM ParserTechnique " +
	        "JOIN ParserField ON ParserField.parserfieldkey = ParserTechnique.parserfieldkey " +
	        ...
	        "WHERE Method.methodkey = ?1 AND ParserTechnique.status = 1",
	        nativeQuery = true)
	List<ParserTechnique> getParserTechniqueByMethodKey(final int methodKey);
