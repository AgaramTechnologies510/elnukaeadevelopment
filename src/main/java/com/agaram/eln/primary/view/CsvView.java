...
//	      				 }
	       				if (dataEntry.getKey().equalsIgnoreCase("createddate")
	       				        || dataEntry.getKey().equalsIgnoreCase("modifieddate")
	       				        || dataEntry.getKey().equalsIgnoreCase("lastloggedon")) {

	       				    String value = String.valueOf(dataEntry.getValue());

	       				    // epoch millis
	       				    if (value.matches("\\d+")) {
	       				        objList.add(dateFormat.format(new Date(Long.parseLong(value))));
	       				    }
	       				    // ISO date string
	       				    else {
	       				        objList.add(dateFormat.format(
