...
public class IotconnectService {
	...
	public List<InstrumentMaster> getInstruments(InstrumentCategory instcat)
	{
		...
		List<InstrumentType> excludedTypes = new ArrayList<>();
		excludedTypes.add(insttype);
		return masterRepo.findByInstcategoryAndStatusAndInsttypeNotIn(instcat,1,excludedTypes);
	}
	
	@Transactional
	public List<RCTCPFileDetails> ConvertRawDataToFile(String rawData,Integer methodkey,Integer nequipmentcode,
			Integer isMultitenant,LSOrderElnMethod orderelnmethod,LSuserMaster userobj,LSSiteMaster site) throws IOException, ParseException, InvalidKeyException, NoSuchAlgorithmException, IllegalArgumentException {
		
		   System.out.println("entered ConvertRawDataToFile service");
	         ...
	         for (List<MethodFieldTechnique> innerList : value) {
	             for (MethodFieldTechnique technique : innerList) {
	                 ...
	                 if(technique.getParseddata().size() == 1) {
	                	 ...
	                	 RCTCPResultFieldValuesRepository.saveAll(resultfieldvaluearray);
	                 ...
	                 if(!technique.getParseddata().isEmpty()) {
		                 if(technique.getParseddata().size() == 1) {
		                	 ...
		                	 for(int i=0;i<fieldresults.size();i++) {
			     			 ...
			     			 System.out.println("result gets inserted here in LSResultFieldValuesRepository");
		                	 LSResultFieldValuesRepository.saveAll(resultfieldvaluearray);
		                	 elnresultdetails.setLsresultfieldvalues(resultfieldvaluearray); 
		          
		                 }
	                 }
	                 ELNResultDetailsRepository.save(elnresultdetails);
	     			 System.out.println("result gets inserted here in ELNResultDetailsRepository");
	             }
	         }
	     }
	}
