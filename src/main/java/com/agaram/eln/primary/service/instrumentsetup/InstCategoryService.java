 ...
 * details from the 'instrumentcategory' table through 'InstrumentCategory'
 * Entity relevant to the input request.
 * 
 * @author ATE153
 * @version 1.0.0
 * @since 11- Nov- 2019
...
//	CfrTransactionService cfrTransService;
	 ...
	 * 
	 * @param category       [InstrumentCategory] - holding details of all fields in
	 *                       'InstrumentCategory' Pojo.
	 * @param saveAuditTrial [boolean] 'true' -to record audit trial, 'false' - not
	 *                       to record audit trial
	 * @param page           [Page] entity relating to 'InstrumentCategory'
	 * @param request        [HttpServletRequest] Request object to ip address of
	 *                       remote client
	 * @return newly added InstrumentCategoryo object with status Message
	 */
	@Transactional
	public ResponseEntity<Object> createInstCategory(final InstrumentCategory category, final boolean saveAuditTrial,
...
//						xmlData, createdUser, request.getRemoteAddr());
	...
	/**
	 * This method is used to update the selected 'category' details based on
	 * provided primary key id and details of the updated fields.
	 * 
...
//			} else {
	...
	public ResponseEntity<Object> getInstCategory(LSSiteMaster lssitemaster) {
		...
		// List<InstrumentCategory> list =
		// categoryRepo.findByStatusAndLssitemaster(1,lssitemaster, new
		// Sort(Sort.Direction.DESC, "instcatkey"));
		List<InstrumentCategory> list = categoryRepo.findByLssitemaster(
			    lssitemaster,
			    Sort.by(Sort.Order.desc("instcatkey"))
			);

//		List<InstrumentCategory> list = categoryRepo.findByLssitemaster(lssitemaster,
//				new Sort.Order(Sort.Direction.DESC, "instcatkey"));
...
//    }
	...
	public ResponseEntity<Object> deleteInstCategory(final Integer categoryKey, final boolean saveAuditTrial,
			final String comments, final Integer userKey, final HttpServletRequest request,
			InstrumentCategory otherdetails, LSSiteMaster site) {
//		Map<String, Object> mapOrders = new HashMap<String, Object>();
		final InstrumentCategory category = categoryRepo.findByInstcatkey(categoryKey);
...
//		ArrayList<String> InstrumentCategory = new ArrayList<String>();
		...
		if (category != null) {
			...
			if (categoryObj.getInstcatkey() == 1) {
				...
				// return new ResponseEntity<>(categoryObj.getInstcatname() + " - Default
				...
				if (instMasterList.size() > 0) {
					...
					// return new ResponseEntity<>(categoryObj.getInstcatname(),
...
//		     						page.getModule().getSite(), xmlData, createdUser, request.getRemoteAddr());
	...
	/**
	 * This method is used to convert the category entity to xml with the difference
	 * in object before and after update for recording in Audit Trial
	 * 
...
//				categoryModified, "individualpojo", fieldMap);
	...
	/**
	 * This method is used to retrieve the 'Users' details based on the input
	 * primary key- userKey.
	 * 
	...
	private LSuserMaster getCreatedUserByKey(final int userKey) {
		final LSuserMaster createdBy = userRepo.findByusercode(userKey);
