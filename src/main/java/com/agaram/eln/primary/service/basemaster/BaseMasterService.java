...
//				.findByLssitemasterAndStatusOrderByProjectcodeDesc(objClass.getLssitemaster(), 1);
	...
	public List<LSprojectmaster> getActiveProjectmaster(LSuserMaster objClass) {
...
//		        LSprojectmaster existingProject = lSprojectmasterRepository.findByProjectcode(objClass.getProjectcode());
...
//		            objClass.setProjectid(existingProject.getProjectid());
//		            
//		            
//		            if (!objorder.isEmpty() ) {
//			        	objClass.getResponse().setStatus(false);
//						objClass.getResponse().setInformation("IDS_PROJECTPROGRESS_SHEETORDER");
//						return objClass;
//		            }else if(!protoorder.isEmpty()) {
//		            	objClass.getResponse().setStatus(false);
//						objClass.getResponse().setInformation("IDS_PROJECTPROGRESS_PROTOCOLORDER");
//						return objClass;
//		            }
//		        }
//		    }
			...
			List<LSlogilablimsorderdetail> objorder = LSlogilablimsorderdetailRepository.findByOrderflagAndLsprojectmasterAndOrdercancellIsNull("N", objClass);

	        List<LSlogilabprotocoldetail> protoorder = logilabprotocoldetailRepository.findByOrderflagAndLsprojectmasterAndOrdercancellIsNull("N", objClass);
...
//projectname edit
			...
			LSprojectmaster objLSprojectmaster = lSprojectmasterRepository.findByProjectcode(objClass.getProjectcode());
		...
		objClass.getResponse().setStatus(true);
		objClass.getResponse().setInformation("");
		if (objClass.getProjectcode() == null) updatenotificationforproject(objClass);
		return objClass;
	}
...
//					.getLsusersteam().getLsuserteammapping().get(i).getLsuserMaster().getUsercode()) {
			...
			if (lsuserMasterRepository.findByusercode(objClass.getLsusermaster().getUsercode()).getUsercode() != objClass
					.getLsusersteam().getLsuserteammapping().get(i).getLsuserMaster().getUsercode()) {
				
				Details = "{\"teamname\":\"" + objClass.getTeamname() + "\", \"projectname\":\""
						+ objClass.getProjectname() + "\"}";

				notify.setNotifationfrom(objClass.getLsusermaster());
				...
				notify.setNotificationfor(2);
				notify.setSitecode(objClass.getLssitemaster().getSitecode());
...
//				.findByLssitemasterOrderByTestcodeDesc(objuser.getLssitemaster());
	...
	public Lselninstrumentmaster InsertupdateInstrument(Lselninstrumentmaster objClass) {
		...
		lselnInstrumentmappingRepository.saveAll(objClass.getLselnInstrumentmapping());
		List<Lselninstrumentfields> objfields = objClass.getLsfields();
		for (Lselninstrumentfields lselninstrumentfields : objfields) {
			lselninstfieldmappingRepository.saveAll(lselninstrumentfields.getLselninstfieldmapping());
...
//			lscfttransactionRepository.save(objClass.getObjsilentaudit());
	...
	public Unit InsertupdateUnit(Unit objClass) {
		...
		if (objClass.getNunitcode() == -1) {
			...
			Unit objUnit = unitRepository.findByNunitcode(objClass.getNunitcode());
	...
	public Map<String, Object> getLsrepositoriesDataLst(Map<String, Object> argMap) {
		...
		if (argMap.containsKey("objsilentaudit")) {
			...
			if (LsrepositoriesLst != null) {
				...
				String repositoryitemname=(String) argMap.get("repositoryitemname");
				List<Lsrepositoriesdata> LsrepositoriesdataLst = LsrepositoriesdataRepository
						.findByRepositoryitemname(repositoryitemname);
				mapObj.put("LsrepositoriesdataLst", LsrepositoriesdataLst);
			}
...
//		List<LSprojectmaster> projectlist = lSprojectmasterRepository
//				.findByLssitemasterAndStatusOrderByProjectcodeDesc(objClass.getLssitemaster(), 1);
		List<LSprojectmaster> prolist = lSprojectmasterRepository.findByLsusersteamInAndStatusAndLssitemaster(LSusersteamRepository
				.findByLsuserteammappingInAndStatusAndLssitemaster(lsuserteammappingRepository.findBylsuserMaster(objClass), 1,objClass.getLssitemaster()), 1,objClass.getLssitemaster());
//		List<LSprojectmaster> projectlist = lSprojectmasterRepository
//				.findByLssitemasterAndStatusOrderByProjectcodeDesc(objClass.getLssitemaster(), 1);
		return prolist;
	}
	...
	{
		return lStestmasterlocalRepository.findByTestcode(objtest.getTestcode());
	}
