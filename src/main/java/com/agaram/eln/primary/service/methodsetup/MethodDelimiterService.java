...
public class MethodDelimiterService {
	...
	public ResponseEntity<Object> getMethodDelimiterList(LSSiteMaster mobj){
		...
		final List<MethodDelimiter> delimiterList = methodDelimiterRepo.findByStatusAndLssitemasterOrLssitemasterIsNull(1,mobj,Sort.by(Sort.Direction.DESC, "methoddelimiterkey"));
	...
	public ResponseEntity<Object> getMethodDelimiter(LSSiteMaster mobj){
		final List<MethodDelimiter> delimiterList = methodDelimiterRepo.findByLssitemasterOrLssitemasterIsNull(mobj,Sort.by(Sort.Direction.DESC, "methoddelimiterkey"));
	...
	public ResponseEntity<Object> createMethodDelimiter(final MethodDelimiter methodDelimiter, final LSSiteMaster site,
		...
		{ 
		   ...
		   final ParserMethod parserMethod = parserMethodRepo.findById(methodDelimiter.getParsermethod().getParsermethodkey()).orElse(null);
		   final Delimiter delimiter = delimiterRepo.findById(methodDelimiter.getDelimiter().getDelimiterkey()).orElse(null);
...
//					   HttpStatus.IM_USED);//status code - 226	
	...
	{
		final LSuserMaster createdBy =  usersRepo.findById(userKey).orElse(null);		
