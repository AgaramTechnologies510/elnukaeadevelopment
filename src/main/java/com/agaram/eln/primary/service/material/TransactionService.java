...
//				lstTypes = materialTypeRepository.findByNmaterialtypecodeNotAndNstatusOrderByNmaterialtypecode(-1, 1);
		...
		lstMaterialInventories.forEach(item -> {
	...
	public ResponseEntity<Object> getInventoryData(Map<String, Object> inputMap) {
		...
		String inventoryname = (String) inputMap.get("inventname");
		Integer nsiteInteger = (Integer) inputMap.get("sitecode");
		
		lstMaterialInventories  = elnmaterialInventoryRepository.findBySequenceidAndNsitecode(inventoryname,nsiteInteger);
		if(lstMaterialInventories.isEmpty()) {
			lstMaterialInventories  = elnmaterialInventoryRepository.findBySinventoryidStartingWithIgnoreCaseAndNsitecode(inventoryname, nsiteInteger);
		}
		
		rtnMap.put("inventories", lstMaterialInventories);
...
//				objTransaction.setCreateddate(objCreatedDate);
		...
		if ((int) insJsonObj.get("ntransactiontype") == Enumeration.TransactionStatus.RETURN.gettransactionstatus()) {
			...
			if (!objList.isEmpty()) {
				...
				resultUsedMaterialRepository.saveAll(objList);
			}
...
//		BigDecimal issuedQtyBD = BigDecimal.valueOf(getIssuedQty);
//		BigDecimal usedQtyBD = BigDecimal.valueOf(getUsedQty);
//		BigDecimal qtyLeftBD = issuedQtyBD.subtract(usedQtyBD);
//		Double getQtyLeft = qtyLeftBD.doubleValue();
//
//		LSuserMaster objUser = new LSuserMaster();
//		objUser.setUsercode(cft.getLsuserMaster());
//
...
//		resultUsedMaterial.setNqtyleft(qtyLeftBD.doubleValue());
...
//		}
//
//		objInventory.setSavailablequantity(qtyLeftBD.toPlainString());
//
//		if (qtyLeftBD.doubleValue() == 0) {
//			objInventory.setNtransactionstatus(-1);
//		}
//
//		resultUsedMaterial.setQtyleft(qtyLeftBD.toPlainString());
...
//						getQtyLeft, new Date()));
//				lsnotificationRepository.saveAll(lstLSnotifications);
//			}
//		}
//
//		return new ResponseEntity<>(resultUsedMaterial, HttpStatus.OK);
//	}
	...
	public ResponseEntity<Object> createMaterialResultUsed(Map<String, Object> inputMap)
		...
		LSprojectmaster projectcode = Objmapper.convertValue(objResultMap.get("projectcode"), LSprojectmaster.class);
		objTest.setTestcode((Integer) objResultMap.get("testcode"));
		Boolean isoldValueGreater = false;
		if (objResultMap.get("isoldValueGreater") != null) {
			isoldValueGreater = (Boolean) objResultMap.get("isoldValueGreater");
		}
		ElnmaterialInventory objInventory = elnmaterialInventoryRepository
				.findByNmaterialinventorycode(objInventoryFromMap.getNmaterialinventorycode());
...
//		Double getIssuedQty = Double.parseDouble(objResultMap.get("issuedQuantity").toString());
		...
		BigDecimal issuedQtyBD = BigDecimal.valueOf(getIssuedQty);
		BigDecimal usedQtyBD = BigDecimal.valueOf(getUsedQty);
		BigDecimal qtyLeftBD;
		if (!isoldValueGreater) {
			if (issuedQtyBD.compareTo(usedQtyBD) < 0
			        || objInventory.getMaterialcategory().getNstatus() == -1
			        || objInventory.getMaterialtype().getNstatus() == -1) {
			    resultUsedMaterial.setResponse(new Response());
			    resultUsedMaterial.getResponse().setStatus(false);
			    return new ResponseEntity<>(resultUsedMaterial, HttpStatus.OK);
			} else {
			    qtyLeftBD = issuedQtyBD.subtract(usedQtyBD);
			    resultUsedMaterial.setIsreturn(0);
			}

		} else {
			qtyLeftBD = issuedQtyBD.add(usedQtyBD);
			resultUsedMaterial.setIsreturn(1);
		}
		qtyLeftBD = qtyLeftBD.setScale(5, RoundingMode.HALF_UP);
		Double getQtyLeft = qtyLeftBD.doubleValue();
		
		if (objInventory.getMaterialcategory().getNstatus() == -1
				|| objInventory.getMaterialtype().getNstatus() == -1) {
			resultUsedMaterial.setResponse(new Response());
			resultUsedMaterial.getResponse().setStatus(false);
			return new ResponseEntity<>(resultUsedMaterial, HttpStatus.OK);
...
//		resultUsedMaterial.setCreateddate(cft.getTransactiondate());
	...
	public ResponseEntity<Object> createMaterialResultUsedReturn(Map<String, Object> inputMap)
		...
		BigDecimal usedQtyBD = BigDecimal.valueOf(getUsedQty);
		BigDecimal availableQtyBD = new BigDecimal(objInventory.getSavailablequantity());
		BigDecimal qtyLeftBD;
...
//			objInventory.setSavailablequantity(getIssuedQty.toString());
			qtyLeftBD = availableQtyBD.add(usedQtyBD);
			getIssuedQty = qtyLeftBD.doubleValue();
			resultUsedMaterial.setNqtyissued(availableQtyBD.doubleValue());
			objInventory.setSavailablequantity(qtyLeftBD.toPlainString());
...
//				objInventory.setSavailablequantity(getIssuedQty.toString());
//			} 
			if (availableQtyBD.compareTo(usedQtyBD) > 0) {
				qtyLeftBD = availableQtyBD.subtract(usedQtyBD);
				getIssuedQty = qtyLeftBD.doubleValue();
				objInventory.setSavailablequantity(qtyLeftBD.toPlainString());
			} else {
				resultUsedMaterial.setResponse(new Response());
...
//		Double savailableQty = previousUsedQuantity + getIssuedQty;
		...
		BigDecimal previousUsedQtyBD = new BigDecimal(inputMap.get("Usedquantity_Copy").toString());
		BigDecimal availableQtyBD = new BigDecimal(objInventory.getSavailablequantity());
		BigDecimal updatedAvailableQtyBD = previousUsedQtyBD.add(availableQtyBD);

		LSuserMaster objUser = new LSuserMaster();
		objUser.setUsercode(cft.getLsuserMaster());

		ElnresultUsedMaterial resultUsedMaterial = new ElnresultUsedMaterial();
		if (objTest.getTestcode() != null && objTest.getTestcode() != -1) {
			resultUsedMaterial.setTestcode(objTest);
		}
		LSprojectmaster projectcode = Objmapper.convertValue(objResultMap.get("projectcode"), LSprojectmaster.class);
...
//		objInventory.setSavailablequantity(savailableQty.toString());
		objInventory.setSavailablequantity(updatedAvailableQtyBD.toPlainString());
...
//		resultUsedMaterial.setQtyleft(savailableQty.toString());
		resultUsedMaterial.setNqtyused(previousUsedQtyBD.doubleValue());
		resultUsedMaterial.setNqtyleft(updatedAvailableQtyBD.doubleValue());
		resultUsedMaterial.setQtyleft(updatedAvailableQtyBD.toPlainString());
		resultUsedMaterial.setBatchid(objResultMap.get("batchid").toString());
		resultUsedMaterial.setNmaterialcode(objInventory.getMaterial().getNmaterialcode());
		resultUsedMaterial.setNmaterialcategorycode(objInventory.getMaterialcategory().getNmaterialcatcode());
		resultUsedMaterial.setNinventorycode(objInventory.getNmaterialinventorycode());
...
//				: objInventory.getSinventoryid();
		...
		sinventoryid = objInventory.getSequenceid() == null ? objInventory.getSinventoryid()
				: objInventory.getSequenceid();
		...
		if (!objLstuser.isEmpty()) {
			...
			if (objnotifyuser.contains(cft.getLsuserMaster())) {
				...
				if (task.equals("INVENTORYQTYNOTIFICATION")) {
					...
					details = "{\"inventoryid\":\"" + sinventoryid + "\",  " + "\"qtyleft\":\"" + getQtyLeft + "\",  "
							+ "\"notificationamount\":\"" + objInventory.getNqtynotification() + "\"}";

				} else if (task.equals("EXPIRYDATE")) {
					details = "{\"inventoryid\":\"" + sinventoryid + "\",  " + "\"daysleft\":\"" + date + "\"}";
				} else {
					details = "{\"inventoryid\":\"" + sinventoryid + "\",  " + "\"daysleft\":\"" + getQtyLeft + "\"}";
				}
				...
				objnotify.setNotificationpath("/material");
				objnotify.setNotificationfor(2);

				if (cft.getLssitemaster() != null) {
					objnotify.setSitecode(cft.getLssitemaster());
				}
	...
	public ResponseEntity<Object> createEquipmentResultUsed(Map<String, Object> inputMap)
		...
		Equipment objInventory = equipmentRepository.findByNequipmentcode(objEquipFromMap.getNequipmentcode());
...
//		Integer sitecode = (Integer) inputMap.get("sitecode");
		...
		lsnotificationRepository.saveAll(lstLSnotifications);
		elnmaterialInventoryRepository.saveAll(expiredInvent);
		SampleRepository.saveAll(expiredsample);
	}
	...
	public List<LSnotification> updateNotificationOnELNSample(Sample objsample, String task, LScfttransaction cft,
		...
		samplename = objsample.getSequenceid() == null ? objsample.getSamplename() : objsample.getSequenceid();
		List<LSnotification> lstnotifications = new ArrayList<LSnotification>();
		...
		if (!objLstuser.isEmpty()) {
			...
			if (objnotifyuser.contains(cft.getLsuserMaster())) {
				...
				if (task.equals("INVENTORYQTYNOTIFICATION")) {
					...
					details = "{\"inventoryid\":\"" + samplename + "\",  " + "\"qtyleft\":\"" + getQtyLeft + "\",  "
							+ "\"notificationamount\":\"" + objsample.getNqtynotification() + "\"}";

				} else if (task.equals("EXPIRYDATE")) {
					details = "{\"inventoryid\":\"" + samplename + "\",  " + "\"daysleft\":\"" + date + "\"}";
				} else {
					details = "{\"inventoryid\":\"" + samplename + "\",  " + "\"daysleft\":\"" + getQtyLeft + "\"}";
				}
			...
			if (getUsrCode != null) {
				...
				if (objnotifyuser.contains(cft.getLsuserMaster())) {
					...
					objnotify.setNotification(task);
					objnotify.setNotificationdetils(notification);
					objnotify.setIsnewnotification(2);
					objnotify.setNotificationpath("/sample");
					objnotify.setNotificationfor(1);
...
//	        Double getQtyLeft = getIssuedQty - getUsedQty;
	...
	public ResponseEntity<Object> getMaterialLst4NewMaterial(Map<String, Object> inputMap)
		...
		if (!lstMaterial.isEmpty()) {
			...
			elnmaterialRepository.saveAll(objLstElnmaterials);
	...
	public ResponseEntity<Object> getMaterialInvLst4NewMaterialInv(Map<String, Object> inputMap)
		...
		if (!lstMaterial.isEmpty()) {
			...
			lstMaterial.stream().peek(objMaterial -> {
				...
				Elnmaterial objElnmaterial = elnmaterialRepository.findByNmaterialcode(objMaterial.getNmaterialcode());
			...
			elnmaterialInventoryRepository.saveAll(objLstElnmaterials);
		}
		return null;
	}
	...
	public ResponseEntity<Object> getEquipmentTransactionResult(Equipment objEquipment) throws ParseException {
		Map<String, Object> objmap = new LinkedHashMap<String, Object>();
		
		Date ToDate =objEquipment.getTodate()!=null? objEquipment.getTodate():commonfunction.getCurrentUtcTime();

		List<ElnresultEquipment> objList = elnresultEquipmentRepository.findByNequipmentcodeAndCreateddateBetween(
				objEquipment.getNequipmentcode(), objEquipment.getFromdate(), ToDate);
		List<EquipmentHistory> lstCal = equipmentHistoryRepository
				.findByNequipmentcodeAndHistorytypeAndCreateddateBetween(objEquipment.getNequipmentcode(), 1,
						objEquipment.getFromdate(), ToDate);
		List<EquipmentHistory> lstMain = equipmentHistoryRepository
				.findByNequipmentcodeAndHistorytypeAndCreateddateBetween(objEquipment.getNequipmentcode(), 2,
						objEquipment.getFromdate(), ToDate);
...
//		Double getIssuedQty = Double.parseDouble(objResultMap.get("issuedQuantity").toString());
		...
		if (ActionType == 0) {
			...
			if (getIssuedQty >= usedQuantity && objSampleobj.getSamplecategory().getNstatus() != -1
					&& objSampleobj.getSampletype().getNstatus() != -1) {
		...
		if (response.getStatus()) {
			Double qtyLeftAsInt = getQtyLeft.doubleValue();
	...
	public ResponseEntity<Object> updateinventorywithoutquantity(Map<String, Object> resultusedmaterial) {
		ObjectMapper Objmapper = new ObjectMapper();
		final ElnmaterialInventory ionvenobj = Objmapper.convertValue(resultusedmaterial.get("elnmaterialinventory"),
				ElnmaterialInventory.class);
		ElnmaterialInventory obj = elnmaterialInventoryReppository
		...
		ElnresultUsedMaterial.setNqtyleft(0D);
		ElnresultUsedMaterial.setCreatedbyusercode(objUser);
		ElnresultUsedMaterial.setNqtyissued(0D);

		ElnresultUsedMaterial.setNqtyused(0D);
		ElnresultUsedMaterial.setNmaterialcode(obj.getMaterial().getNmaterialcode());
		ElnresultUsedMaterial.setNmaterialcategorycode(obj.getMaterialcategory().getNmaterialcatcode());
		ElnresultUsedMaterial.setNinventorycode(obj.getNmaterialinventorycode());
		ElnresultUsedMaterial.setNmaterialtypecode(obj.getMaterialtype().getNmaterialtypecode());
