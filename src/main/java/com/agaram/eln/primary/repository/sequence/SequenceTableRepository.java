...
public interface SequenceTableRepository extends JpaRepository<SequenceTable, Integer>{
	...
	@Query("update SequenceTable s set s.resetperiod = ?1, s.sequenceview = ?2, "
			+ "s.sequenceformat = ?3, s.seperator = ?4, s.sequencemonth = ?6 , s.sequenceyear = ?7  where s.sequencecode = ?5")
	void updatesequencedata(Integer resetperiod, Integer sequenceview, String sequenceformat,String seperator, Integer sequencecode , Integer sequencemonth , Integer sequenceyear);

	SequenceTable findBySequencecodeOrderBySequencecode(int i);
	SequenceTable findBySequencecode(int i);
}
