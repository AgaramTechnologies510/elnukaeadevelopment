...
//	public  List<LSfile> findByFilenameuserNotAndRejectedNot(String filenameuser, Integer reject);
	...
	@Transactional
	@Modifying
	@Query("update LSfile set lssheetworkflow= :workflow, approved= :approved, rejected= :rejected , modifieddate = :modifieddate , previousapprovedworkflow =:lSsheetworkflow2 , workflowapprovedusercode =:integer  where filecode in (:filecode)")
	public void updateFileWorkflow(@Param("workflow")  LSsheetworkflow lssheetworkflow, @Param("approved")  Integer approved, @Param("rejected")  Integer rejected, @Param("filecode")  
	Integer filecode,@Param("modifieddate") Date modifieddate,@Param("lSsheetworkflow2") LSsheetworkflow lSsheetworkflow2,@Param("integer") Integer integer);
...
//	@Query(value ="SELECT filecode, filenameuser " + 
...
//			"ORDER BY filecode DESC", nativeQuery = true)	
	@Query("SELECT new com.agaram.eln.primary.model.sheetManipulation.LSfile(l.filecode, l.filenameuser,l.versionno) " +
		       "FROM LSfile l " +
		       "WHERE l.retirestatus = 0 " +
		       "  AND l.lssitemaster = ?1 " +
		       "  AND ((l.filecode > 1 AND l.approved != 1) OR l.approved IS NULL) " +
		       "  AND (l.rejected != 1 OR l.versionno > 1) " +
		       "ORDER BY l.filecode DESC")
		List<LSfile> getsheetGreaterthanoneandapprovel(LSSiteMaster lsSiteMaster);
	
	@Transactional
	@Modifying
	@Query(value ="select filecode, filenameuser  from LSfile where lssitemaster_sitecode=?3 and filecode >1 and approved= ?1 or versionno > 1 and rejected=0 and createby in (?2) ORDER BY filecode DESC",nativeQuery=true)
...
//	@Query(value="SELECT SELECT filecode, filenameuser  " + 
//			"FROM com.agaram.eln.primary.model.sheetManipulation.LSfile " + 
//			"WHERE retirestatus=0 AND lssitemaster_sitecode = ?2 AND " + 
//			"      ((filecode > 1 AND approved != 1 AND rejected != 1) OR approved IS NULL) AND " + 
//			"      ((rejected != 1) OR (versionno > 1)) " + 
//			" and createby in (?1)ORDER BY filecode DESC",nativeQuery=true)
//	public List<LSfile> getsheetapprovelanduserIn(List<LSuserMaster> lstusermaster,Integer sitecode);
	
	
	
//	@Query("SELECT filecode, filenameuser " +
//		       "FROM com.agaram.eln.primary.model.sheetManipulation.LSfile " +
//		       "WHERE " +
//		       "  (retirestatus = 0 AND lssitemaster = ?2 " +
//		       "   AND ((filecode > 1 AND approved != 1 AND rejected != 1) OR approved IS NULL) " +
//		       "   AND (rejected != 1 OR versionno > 1) " +
//		       "   AND createby IN (?1)) " +
//		       "OR " +
//		       "  (retirestatus = 0 AND lssitemaster = ?2 " +
//		       "   AND ((filecode > 1 AND approved != 1 AND rejected != 1) OR approved IS NULL) " +
//		       "   AND (rejected != 1 OR versionno > 1) " +
//		       "   AND createby NOT IN (?1) " +
//		       "   AND viewoption = 1) " +
//		       "ORDER BY filecode DESC")
//		public List<LSfile> getsheetapprovelanduserIn(List<LSuserMaster> usercodes, LSSiteMaster lsSiteMaster);
	
	@Query("SELECT new com.agaram.eln.primary.model.sheetManipulation.LSfile(l.filecode, l.filenameuser,l.versionno) " +
		       "FROM LSfile l " +
		       "WHERE " +
		       "  (l.retirestatus = 0 AND l.lssitemaster = ?2 " +
		       "   AND ((l.filecode > 1 AND l.approved != 1 AND l.rejected != 1) OR l.approved IS NULL) " +
		       "   AND (l.rejected != 1 OR l.versionno > 1) " +
		       "   AND l.createby IN (?1)) " +
		       "OR " +
		       "  (l.retirestatus = 0 AND l.lssitemaster = ?2 " +
		       "   AND ((l.filecode > 1 AND l.approved != 1 AND l.rejected != 1) OR l.approved IS NULL) " +
		       "   AND (l.rejected != 1 OR l.versionno > 1) " +
		       "   AND l.createby NOT IN (?1) " +
		       "   AND l.viewoption = 1) " +
		       "ORDER BY l.filecode DESC")
		List<LSfile> getsheetapprovelanduserIn(List<LSuserMaster> usercodes, LSSiteMaster lsSiteMaster);


	
	
	@Query("SELECT new com.agaram.eln.primary.model.sheetManipulation.LSfile(l.filecode, l.filenameuser) " + 
			"FROM LSfile l " +
			"WHERE l.retirestatus = 0 And (l.tagsheet = 1 OR l.resultsheet = 1) AND l.lssitemaster = ?2 AND " + 
			"      (l.filecode > 1 AND l.approved = 1 AND l.rejected != 1) AND " + 
			"      (l.rejected != 1) " + 
			" and l.createby in (?1)ORDER BY l.filecode DESC")
	public List<LSfile> gettemplateapprovelanduserIn(List<LSuserMaster> lstusermaster,LSSiteMaster lsSiteMaster);
...
//	public List<LSfile> findByFilecodeOrderByFilecodeDesc(List<Integer> listobjfilecode);
...
//	public Object countByCreatebyIn(Integer sitecode);
	...
	public Object countByFilecodeGreaterThanAndLssitemasterAndViewoptionOrFilecodeGreaterThanAndCreatebyAndViewoptionOrderByFilecodeDesc(
...
//			Date fromdate2, Date todate2, int m, int n);
	...
	public List<Sheettemplateget> findByCreatedateBetweenAndFilecodeGreaterThanAndLssitemasterAndViewoptionOrCreatedateBetweenAndFilecodeGreaterThanAndCreatebyAndViewoptionOrderByFilecodeDesc(
	...
	public long countByCreatedateBetweenAndFilecodeGreaterThanAndLssitemasterAndViewoptionOrCreatedateBetweenAndFilecodeGreaterThanAndCreatebyAndViewoptionOrderByFilecodeDesc(
			Date fromdate, Date todate, int i, LSSiteMaster lssitemaster, int j, Date fromdate2, Date todate2, int k,
...
//	
	...
	public List<Sheettemplateget> findByFilecodeGreaterThanAndLssitemasterAndViewoptionOrFilecodeGreaterThanAndCreatebyInAndViewoptionAndLssitemasterOrderByFilecodeDesc(
			int i, LSSiteMaster lssitemaster, int j, int k, List<LSuserMaster> objuser, int l, LSSiteMaster lssitemaster2);

	public List<Sheettemplateget> findByFilecodeGreaterThanAndLssitemasterAndViewoptionOrFilecodeGreaterThanAndCreatebyAndViewoptionAndLssitemasterOrFilecodeGreaterThanAndCreatebyInAndViewoptionAndLssitemasterOrderByFilecodeDesc(
			int i, LSSiteMaster lssitemaster, int j, int k, LSuserMaster objuser, int l, LSSiteMaster lssitemaster2,
			int m, List<LSuserMaster> lstteamuser, int n, LSSiteMaster lssitemaster3);

	public Object countByFilecodeGreaterThanAndLssitemasterAndViewoptionOrFilecodeGreaterThanAndCreatebyAndViewoptionAndLssitemasterOrFilecodeGreaterThanAndCreatebyInAndViewoptionAndLssitemasterOrderByFilecodeDesc(
			int i, LSSiteMaster lssitemaster, int j, int k, LSuserMaster objusers, int l, LSSiteMaster lssitemaster2,
			int m, List<LSuserMaster> lstteamuser, int n, LSSiteMaster lssitemaster3);

	public Object countByFilecodeGreaterThanAndLssitemasterAndViewoptionOrFilecodeGreaterThanAndCreatebyAndViewoptionAndLssitemasterOrderByFilecodeDesc(
			int i, LSSiteMaster lssitemaster, int j, int k, LSuserMaster objusers, int l, LSSiteMaster lssitemaster2);

	public List<LSfile> (
			int i, int j, LSSiteMaster lssitemaster, int k, int l, Integer approvelstatus, int m, int n,
			LSuserMaster objuser, LSSiteMaster lssitemaster2, int o, int p, Integer approvelstatus2, int q, int r,
			List<LSuserMaster> lstteamuser, LSSiteMaster lssitemaster3, int s, int t, Integer approvelstatus3);

	public List<LSfile> findByFilecodeGreaterThanAndRetirestatusAndLssitemasterAndViewoptionAndRejectedAndApprovedOrFilecodeGreaterThanAndRetirestatusAndCreatebyInAndLssitemasterAndViewoptionAndRejectedAndApprovedOrderByFilecodeDesc(
			int i, int j, LSSiteMaster lssitemaster, int k, int l, Integer approvelstatus, int m, int n,
			List<LSuserMaster> lstteamuser,LSSiteMaster lssitemaster2,  int o, int p, Integer approvelstatus2);

	public List<LSfile> findByFilecodeGreaterThanAndRetirestatusAndLssitemasterAndViewoptionAndRejectedAndApprovedOrFilecodeGreaterThanAndRetirestatusAndCreatebyAndLssitemasterAndViewoptionAndRejectedAndApprovedOrderByFilecodeDesc(
			int i, int j, LSSiteMaster lssitemaster, int k, int l, Integer approvelstatus, int m, int n,
			LSuserMaster objuser, LSSiteMaster lssitemaster2, int o, int p, Integer approvelstatus2);

	public List<LSfile> findByFilecodeGreaterThanAndLssitemasterAndViewoptionOrFilecodeGreaterThanAndCreatebyAndViewoptionAndLssitemasterOrFilecodeGreaterThanAndCreatebyInAndViewoptionAndLssitemaster(
			int i, LSSiteMaster lssitemaster, int j, int k, LSuserMaster objuser, int l, LSSiteMaster lssitemaster2,
			int m, List<LSuserMaster> lstteamuser, int n, LSSiteMaster lssitemaster3);

	public List<LSfile> findByFilecodeGreaterThanAndLssitemasterAndViewoptionOrFilecodeGreaterThanAndCreatebyInAndViewoptionAndLssitemaster(
			int i, LSSiteMaster lssitemaster, int j, int k, List<LSuserMaster> lstteamuser1, int l,
			LSSiteMaster lssitemaster2);

	public long countByLssheetworkflowAndApprovedNotAndRejectedNotOrApprovedIsNull(LSsheetworkflow objflow, int i,
			int j);

	public long countByLssheetworkflowAndApprovedAndRejectedAndRetirestatusOrLssheetworkflowAndApprovedIsNullAndRejectedAndRetirestatus(
			LSsheetworkflow objflow, int i, int j, int k, LSsheetworkflow objflow2, int l, int m);

	public long countByLssitemasterAndLssheetworkflowIsNullAndApprovedIsNullAndRejectedAndRetirestatus(
			LSSiteMaster lssitemaster, int i, int j);

	public List<LSfile> findByLssitemasterAndLssheetworkflowIsNullAndApprovedIsNullAndRejectedAndRetirestatus(
			LSSiteMaster lssitemaster, int i, int j);
}