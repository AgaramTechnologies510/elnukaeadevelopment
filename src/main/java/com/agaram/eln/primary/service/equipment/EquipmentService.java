...
//	private LogilablimsorderdetailsRepository LogilablimsorderdetailsRepository;
	...
	public ResponseEntity<Object> getEquipment(Map<String, Object> inputMap) throws ParseException {
		...
		lstEquipment.forEach(item -> {
			...
			if(item.getSequenceid() == null) {
				item.setSequenceid(item.getSequipmentid());
	...
	public ResponseEntity<Object> getEquipmentOnTransaction(Map<String, Object> inputMap) throws ParseException {
		...
		List<Equipment> lstEquipment = equipmentRepository
				.findByEquipmentusedAndNsitecodeAndNstatusAndCreateddateBetweenAndLastmaintainedNotNullAndLastcallibratedNotNullOrderByNequipmentcodeDesc(
						true, nsiteInteger, 1, fromDate, toDate);
		
		lstEquipment.addAll(equipmentRepository
				.findByNsitecodeAndCreateddateBetweenAndEquipmentusedAndNstatusOrderByNequipmentcodeDesc(nsiteInteger, fromDate, toDate,true,1));

		lstEquipment.addAll(equipmentRepository
				.findByEquipmentusedAndNsitecodeAndNstatusAndReqcalibrationAndLastmaintainedNotNullOrderByNequipmentcodeDesc(
						true, nsiteInteger, 1, false));
		lstEquipment.addAll(equipmentRepository
				.findByEquipmentusedAndNsitecodeAndNstatusAndReqmaintananceAndLastcallibratedNotNullOrderByNequipmentcodeDesc(
						true, nsiteInteger, 1, false));
		lstEquipment.addAll(equipmentRepository
				.findByEquipmentusedAndNsitecodeAndNstatusAndReqmaintananceAndReqcalibrationOrderByNequipmentcodeDesc(
						true, nsiteInteger, 1, false, false));
	...
	public ResponseEntity<Object> getEquipmentPropsForFilter(Integer nsiteInteger) {
		...
		lstTypes = equipmentTypeRepository
	...
	public ResponseEntity<Object> getEquipmentByFilter(Map<String, Object> inputMap) {
		...
		EquipmentCategory objCategory = objmapper.convertValue(inputMap.get("equipmentcategory"),
		...
		if ((objType == null || objType.getNequipmenttypecode() == null || objType.getNequipmenttypecode() == -1)
				&& (objCategory == null || objCategory.getNequipmentcatcode() == null
						|| objCategory.getNequipmentcatcode() == -1)) {
			lstEquipment = equipmentRepository
					.findByNsitecodeAndNstatusAndEquipmentusedAndCreateddateBetweenOrderByNequipmentcodeDesc(nsiteInteger,1,true, fromDate, toDate);
			...
			lstEquipment = equipmentRepository
					.findByEquipmentcategoryAndNstatusAndEquipmentusedAndNsitecodeAndCreateddateBetweenOrderByNequipmentcodeDesc(objCategory,1,true,
							nsiteInteger, fromDate, toDate);
		} else if (objType != null && objType.getNequipmenttypecode() != -1 && (objCategory == null
				|| objCategory.getNequipmentcatcode() == null || objCategory.getNequipmentcatcode() == -1)) {
			lstEquipment = equipmentRepository
					.findByEquipmenttypeAndNstatusAndEquipmentusedAndNsitecodeAndCreateddateBetweenOrderByNequipmentcodeDesc(objType,1,true,
							nsiteInteger, fromDate, toDate);
		} else {
			lstEquipment = equipmentRepository
					.findByEquipmenttypeAndNstatusAndEquipmentusedAndEquipmentcategoryAndNsitecodeAndCreateddateBetweenOrderByNequipmentcodeDesc(
							objType,1,true, objCategory, nsiteInteger, fromDate, toDate);
		}
	...
	public ResponseEntity<Object> getEquipmentProps(Integer nsiteInteger) {
		...
		lstTypes = equipmentTypeRepository
				.findByNequipmenttypecodeNotAndNstatusAndNsitecodeOrderByNequipmenttypecodeDesc(-1, 1, nsiteInteger);
		if (!lstTypes.isEmpty()) {
			lstCategories = equipmentCategoryRepository
					.findByNsitecodeAndNstatusOrderByNequipmentcatcodeDesc(nsiteInteger, 1);
		}
	...
	public ResponseEntity<Object> getEquipmentTransactionProps(Integer nsiteInteger) {
		...
		if (!lstTypes.isEmpty()) {
			lstCategories = equipmentCategoryRepository
					.findByEquipmenttypeAndNsitecodeAndNstatusOrderByNequipmentcatcodeDesc(lstTypes.get(0),
							nsiteInteger, 1);
			if (!lstCategories.isEmpty()) {
				lstEquipments = equipmentRepository
						.findByEquipmentusedAndEquipmentcategoryAndNsitecodeAndNstatusAndLastmaintainedNotNullAndLastcallibratedNotNullOrderByNequipmentcodeDesc(
								true, lstCategories.get(0), nsiteInteger, 1);
				lstEquipments.addAll(equipmentRepository
						.findByEquipmentusedAndEquipmentcategoryAndNsitecodeAndNstatusAndReqcalibrationAndLastmaintainedNotNullOrderByNequipmentcodeDesc(
								true, lstCategories.get(0), nsiteInteger, 1, false));
				lstEquipments.addAll(equipmentRepository
						.findByEquipmentusedAndEquipmentcategoryAndNsitecodeAndNstatusAndReqmaintananceAndLastcallibratedNotNullOrderByNequipmentcodeDesc(
								true, lstCategories.get(0), nsiteInteger, 1, false));
				lstEquipments.addAll(equipmentRepository
						.findByEquipmentusedAndEquipmentcategoryAndNsitecodeAndNstatusAndReqmaintananceAndReqcalibrationOrderByNequipmentcodeDesc(
								true, lstCategories.get(0), nsiteInteger, 1, false, false));
			}
		}
		...
		List<Equipment> sortedList = lstEquipments.stream()
		        .sorted(Comparator.comparing(Equipment::getNequipmentcode).reversed())
		        .collect(Collectors.toList());

		objmap.put("lstEquipments", sortedList);
		objmap.put("lstCategories", lstCategories);
		objmap.put("lstType", lstTypes);
	...
	public ResponseEntity<Object> getEquipmentTypeBasedCat(Map<String, Object> inputMap) {
		...
		if (ntypecode == -1) {
			...
			if (!lstTypes.isEmpty()) {
				...
				if (!lstCategories.isEmpty()) {
					lstEquipments = equipmentRepository
							.findByEquipmentusedAndEquipmentcategoryAndNsitecodeAndNstatusOrderByNequipmentcodeDesc(
									true, lstCategories.get(0), nsiteInteger, 1);
				}
			}
		}
	...
	public ResponseEntity<Object> getEquipmentTypeBasedCatOnTrans(Map<String, Object> inputMap) {
		...
		if (ntypecode == -1) {
			...
			if (!lstTypes.isEmpty()) {
				lstCategories = equipmentCategoryRepository.findByEquipmenttypeAndNsitecodeAndNstatusOrderByNequipmentcatcodeDesc(lstTypes.get(0),
						nsiteInteger, 1);
				if (!lstCategories.isEmpty()) {
					lstEquipments = equipmentRepository
							.findByEquipmentusedAndEquipmentcategoryAndNsitecodeAndNstatusAndLastmaintainedNotNullAndLastcallibratedNotNullOrderByNequipmentcodeDesc(
...
//	         .collect(Collectors.toList());
	...
	public ResponseEntity<Object> getEquipmentCatBased(Map<String, Object> inputMap) {
		...
		lstEquipments = equipmentRepository.findByEquipmentusedAndEquipmentcategoryAndNsitecodeAndNstatus(true,
				objCategory, nsiteInteger, 1);

		objmap.put("lstEquipments", lstEquipments);
		return new ResponseEntity<>(objmap, HttpStatus.OK);
	}

	public void GetEquipSequence(Equipment objequip, SequenceTable seqequip, SequenceTableProjectLevel objprojectseq,
		...
		if (sqa != null) {
			...
			if (objequip != null && objequip.getNsitecode() != null && objequip.getNsitecode() != null
			...
			if (sequence.contains("{s&") && sequence.contains("$s}")) {
				...
				String replacedseq = "";
				if (sqa.getSequenceview().equals(2) && objequip.getApplicationsequence() != null
						&& !sequencetext.equals("")) {
					replacedseq = String.format("%0" + sequencetext.length() + "d", objequip.getApplicationsequence());
				} else if (sqa.getSequenceview().equals(3) && objequip.getSitesequence() != null
						&& !sequencetext.equals("")) {
			...
			if (sequence.contains("{m&") && sequence.contains("$m}")) {
				...
				String namedmonth = sequencetext.substring(sequencetext.indexOf("{m&") + 3,
						sequencetext.indexOf("$m}"));

				if (!namedmonth.equals("")) {
					sequencetext = sequencetext.substring(0, sequencetext.indexOf("{m&")) + currentMonth
							+ sequencetext.substring(sequencetext.indexOf("$m}") + 3, sequencetext.length());
				}
			}
			...
			if (sequence.contains("{mm&") && sequence.contains("$mm}")) {
				...
				String currentMonth = month.format(currentdate);
				String namedmonth = sequencetext.substring(sequencetext.indexOf("{mm&") + 4,
						sequencetext.indexOf("$mm}"));

				if (!namedmonth.equals("")) {
					sequencetext = sequencetext.substring(0, sequencetext.indexOf("{mm&")) + currentMonth
							+ sequencetext.substring(sequencetext.indexOf("$mm}") + 4, sequencetext.length());
				}
			}
			...
			if (sequence.contains("{dd&") && sequence.contains("$dd}")) {
				...
				String namedday = sequencetext.substring(sequencetext.indexOf("{dd&") + 4,
						sequencetext.indexOf("$dd}"));

				if (!namedday.equals("")) {
					sequencetext = sequencetext.substring(0, sequencetext.indexOf("{dd&")) + currentMonth
							+ sequencetext.substring(sequencetext.indexOf("$dd}") + 4, sequencetext.length());
				}
			}
			...
			if (sequence.contains("{y&") && sequence.contains("$y}")) {
				...
				if (!namedyear.equals("")) {
					sequencetext = sequencetext.substring(0, sequencetext.indexOf("{y&")) + currentMonth
							+ sequencetext.substring(sequencetext.indexOf("$y}") + 3, sequencetext.length());
				}
			}
			...
			if (sequence.contains("{yy&") && sequence.contains("$yy}")) {
				...
				String currentMonth = year.format(currentdate);
				String namedyear = sequencetext.substring(sequencetext.indexOf("{yy&") + 4,
						sequencetext.indexOf("$yy}"));

				if (!namedyear.equals("")) {
					sequencetext = sequencetext.substring(0, sequencetext.indexOf("{yy&")) + currentMonth
							+ sequencetext.substring(sequencetext.indexOf("$yy}") + 4, sequencetext.length());
				}
			}
	...
	public SequenceTable validateandupdateequipmentsequencenumber(Equipment obj,
		...
		int sequence = 4;
		seqorder = sequencetableRepository.findBySequencecodeOrderBySequencecode(sequence);
...
//		if(objEquipment == null) {
		...
		String stridformat = returnSubstring(obj.getSequipmentname()) + "/"
				+ getfnFormat(obj.getNequipmentcode(), sformattype);
...
//		}
	...
	public ResponseEntity<Object> updateELNEquipment(Equipment obj) throws ParseException {
		...
		Equipment objElnmaterial = equipmentRepository
				.findByNsitecodeAndSequipmentnameAndEquipmentcategoryAndNequipmentcodeNotAndNstatus(obj.getNsitecode(),
						obj.getSequipmentname(), obj.getEquipmentcategory(), obj.getNequipmentcode(), 1);

		obj.setResponse(new Response());

		if (objElnmaterial == null) {

			Equipment objMaterial = equipmentRepository.findByNequipmentcode(obj.getNequipmentcode());
			...
			if(objMaterial.getCreateby() != null) {
				obj.setCreateby(objMaterial.getCreateby());
			}
			...
			if (Boolean.FALSE.equals(obj.getCmmsetting()) && objMaterial.getCommunicationsetting() != null) {
				 ...
				 */
				communicationRepository.findByCmmsettingcode(cmsTemp.getCmmsettingcode())
						.ifPresent(communicationRepository::delete);
	...
	public ResponseEntity<Object> updateEquipment(Equipment obj) {
		...
		Equipment objEquipment = equipmentRepository
				.findByNsitecodeAndSequipmentnameAndEquipmentcategoryAndNequipmentcodeNot(obj.getNsitecode(),
						obj.getSequipmentname(), obj.getEquipmentcategory(), obj.getNequipmentcode());

		obj.setResponse(new Response());

		if (objEquipment == null) {

			Equipment objClass = equipmentRepository.findByNequipmentcode(obj.getNequipmentcode());
	...
	public ResponseEntity<Object> updateStatus(Equipment obj) {
		...
		Equipment objEquipment = equipmentRepository.findByNequipmentcode(obj.getNequipmentcode());
	...
	public ResponseEntity<Object> getEquipmentBySearchField(Map<String, Object> inputMap) {
		...
		List<Equipment> lstEquipments = equipmentRepository
				.findBySequipmentnameStartingWithIgnoreCaseAndNsitecode(searchString, nsiteInteger);
	...
	public ResponseEntity<Object> OsearchEquipment(Map<String, Object> inputMap) {
		...
		String searchString = (String) inputMap.get("searchString");
		List<Equipment> lstEquipments = equipmentRepository
				.findBySequipmentnameStartingWithIgnoreCaseAndNsitecodeOrderByNequipmentcodeDesc(searchString,
						nsiteInteger);
		objmap.put("lstEquipment", lstEquipments);
		return new ResponseEntity<>(objmap, HttpStatus.OK);
	}
	...
	public ResponseEntity<Object> onGetEquipmentSelect(Map<String, Object> inputMap)
		...
		if(inputMap.get("isupdateresult")!=null &&  (Boolean) inputMap.get("isupdateresult")) {
			isupdateresult =false;
		}
		Integer Status =inputMap.get("status") !=null ? (Integer) inputMap.get("status"):1;
		List<Equipment> lstEquipments = equipmentRepository.findByNequipmentcodeIn(nequipmentcode);
		objmap.put("lstEquipment", lstEquipments);

		if ((!lstEquipments.isEmpty() && selectedScreen != null && selectedScreen == 2) || (Status==3 && isupdateresult)) {
			LSuserMaster objUser = new LSuserMaster();
			objUser.setUsercode(user);
			LSlogilabprotocoldetail objDetail = lslogilabprotocoldetailRepository
					.findByProtocolordercode(Long.valueOf(objResultMap.get("protocolordercode").toString()));
			if (isupdateresult) {
				createEquipmentResultUsedLst(objDetail, objUser, lstEquipments, Status);
			}
		}
	...
	public void createEquipmentResultUsedLst(LSlogilabprotocoldetail objDetail, LSuserMaster objUser,
			List<Equipment> lstEquipments, Integer status)
			throws JsonParseException, JsonMappingException, IOException {
		List<ElnresultEquipment> lstEquipments2 = new ArrayList<ElnresultEquipment>();
		List <ElnresultEquipment> existedEquipments = new ArrayList<ElnresultEquipment>();
		if(status!=null && (status==2 || status==3)) {
			List<Integer> nequipmentcode = lstEquipments.stream().map(Equipment::getNequipmentcode).collect(Collectors.toList());
			if(status==3) {
				existedEquipments = elnresultEquipmentRepository.findByOrdercodeAndIscompleteAndIsreturnedAndTemplatecodeIsNotNullAndTemplatecodeIsNotOrderByNresultequipmentcodeDesc(Long.valueOf(objDetail.getProtocolordercode()),false,false,-1);
			}else {
				existedEquipments = elnresultEquipmentRepository.findByOrdercodeAndIscompleteAndIsreturnedAndTemplatecodeIsNotNullAndTemplatecodeIsNotAndNequipmentcodeInOrderByNresultequipmentcodeDesc(Long.valueOf(objDetail.getProtocolordercode()),false,false,-1,nequipmentcode);

			}
		}
		if(existedEquipments!=null && !existedEquipments.isEmpty()) {
			if(status==2) {
				ElnresultEquipment oneequipment = existedEquipments.get(0);
				oneequipment.setIsreturned(true);
				lstEquipments2.add(oneequipment);
			}else if(status==3) {
				existedEquipments.stream().forEach(objInventory -> {
					objInventory.setIscomplete(true);
					lstEquipments2.add(objInventory);
				});
			}
		}else {
			lstEquipments.stream().peek(objInventory -> {
				...
				resultEquipment.setNstatus(status);
				resultEquipment.setResponse(new Response());
				resultEquipment.getResponse().setStatus(true);
				resultEquipment.setIscomplete(false);
				resultEquipment.setIsreturned(false);
				try {
		...
		if (!lstEquipments2.isEmpty()) {
			elnresultEquipmentRepository.saveAll(lstEquipments2);
		}
	}
	...
	public Boolean onCheckItsUsed(Equipment objEquipment) {
		...
		List<ElnresultEquipment> objEquipments = elnresultEquipmentRepository
				.findByNequipmentcodeAndAndIscompleteAndIsreturned(objEquipment.getNequipmentcode(),false,false);

		if (!objEquipments.isEmpty()) {

			List<Long> batchcode = objEquipments.stream().map(ElnresultEquipment::getOrdercode).distinct()
					.collect(Collectors.toList());

			List<LSlogilablimsorderdetail> lstOrders = lslogilablimsorderdetailRepository
					.findByBatchcodeInAndOrderflag(batchcode, "N");
			List<LSlogilabprotocoldetail> lstPro = lslogilabprotocoldetailRepository
					.findByProtocolordercodeInAndOrderflag(batchcode, "N");
			List<Lslogbooks> lstLog = lslogbooksRepository.findByLogbookcodeIn(batchcode);
...
//		if (!objEquipments.isEmpty()) {
//
//			List<String> batchid = objEquipments.stream().map(ElnresultEquipment::getBatchid).distinct()
//					.collect(Collectors.toList());
//
//			List<LSlogilablimsorderdetail> lstOrders = LogilablimsorderdetailsRepository
//					.findByBatchidInAndOrderflag(batchid, "N");
//			List<LSlogilabprotocoldetail> lstPro = lslogilabprotocoldetailRepository
//					.findByProtoclordernameInAndOrderflag(batchid, "N");
...
//		}
	...
	public Boolean updatesequenceondefault(Equipment obj) {
		Equipment objEquipments = equipmentRepository.findByNequipmentcode(obj.getNequipmentcode());

		if (objEquipments != null) {
			objEquipments.setSequenceid(objEquipments.getSequipmentid());
			equipmentRepository.save(objEquipments);
		}
		return true;
	}

	public ResponseEntity<Map<String, Object>> EquipmentImportDatatoStore(Map<String, Object> inputMap)
			throws ParseException {
		ObjectMapper obj = new ObjectMapper();
		
		Date currentDate = commonfunction.getCurrentUtcTime();
		Map<String, Object> responseMap = new HashMap<>();
		List<Equipment> obEquipmentLst = obj.convertValue(inputMap.get("Equipment"), new TypeReference<List<Equipment>>() { });
		
		if (!obEquipmentLst.isEmpty()) {
					
			List<String> lstEquipmentNames = obEquipmentLst.stream().map(Equipment::getSequipmentname).collect(Collectors.toList());
			
			Map<String, Long> existEquList = materialCategoryService.checkDeblicaterecord(lstEquipmentNames, obEquipmentLst.get(0).getNsitecode(), "equipment", "sequipmentname");
			
			obEquipmentLst = obEquipmentLst.stream().map(itemsv -> {
				itemsv.setCreateddate(currentDate);
				
				if (!existEquList.isEmpty() && existEquList.get(itemsv.getSequipmentname()) > 0) {
					Long existcount = existEquList.get(itemsv.getSequipmentname()) + 1;
					itemsv.setSequipmentname(itemsv.getSequipmentname() + "(" + existcount + ")");
				}
				
				return itemsv;
			}).filter(Objects::nonNull).collect(Collectors.toList());
//			equipmentRepository.save(Equipment);
			if (obEquipmentLst != null && obEquipmentLst.size() > 0) {
				Collections.reverse(obEquipmentLst);
				equipmentRepository.saveAll(obEquipmentLst);
				SequenceTableProjectLevel objprojectseq = new SequenceTableProjectLevel();
				SequenceTableTaskLevel objtaskseq = new SequenceTableTaskLevel();
				SequenceTable seqorder = validateandupdateequipmentsequencenumber(obEquipmentLst.get(0), objprojectseq, objtaskseq);
				boolean isrest = false;
				seqorder = commonfunction.ResetSequence(seqorder, isrest);
				SequenceTable seqordercopy=seqorder;
				String sformattype = "{yyyy}/{99999}";
//				Collections.reverse(Equipment);
				obEquipmentLst.stream().map((items) -> {
					try {
						GetEquipSequence(items, seqordercopy, objprojectseq, objtaskseq);
						seqordercopy.setApplicationsequence(seqordercopy.getApplicationsequence() + 1);
						if (items != null && items.getNsitecode() != null) {
							SequenceTableSite sqsite = seqordercopy.getSequencetablesite().stream()
									.filter(sq -> sq.getSitecode().equals(items.getNsitecode())
											&& sq.getSequencecode().equals(seqordercopy.getSequencecode()))
									.findFirst().orElse(null);
							if (sqsite != null) {
					...
					try {
						String stridformat = returnSubstring(items.getSequipmentname()) + "/" + getfnFormat(items.getNequipmentcode(), sformattype);
						items.setSequipmentid(stridformat);
					} catch (Exception e) {
						e.printStackTrace();
					}
				...
				equipmentRepository.saveAll(obEquipmentLst);
				responseMap.put("Equipment", obEquipmentLst);
				updatesequence(4,  obEquipmentLst.get(obEquipmentLst.size() - 1));
			}
		}
