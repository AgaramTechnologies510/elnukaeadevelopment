...
public class AuditService {
	...
	public List<LScfrreasons> getreasons(Map<String, Object> objMap) {
		...
		LSSiteMaster lsSiteMaster = objMapper.convertValue(objMap.get("lssitemaster"), LSSiteMaster.class);
		List<LScfrreasons> result = new ArrayList<LScfrreasons>();
		
		result = LScfrreasonsRepository.findByLssitemasterOrderByReasoncodeDesc(lsSiteMaster);
		return result;
	}
	
	public List<LScfrreasons> GetActiveReasons(Map<String, Object> objMap) {
		ObjectMapper objMapper = new ObjectMapper();
		LSSiteMaster lsSiteMaster = objMapper.convertValue(objMap.get("lssitemaster"), LSSiteMaster.class);
		List<LScfrreasons> result = new ArrayList<LScfrreasons>();
		
		result = LScfrreasonsRepository.findByLssitemasterAndStatus(lsSiteMaster, 1);
		return result;
	}
	...
	public LScfrreasons InsertupdateReasons(LScfrreasons objClass) {
		LScfrreasons lScfrreasons = LScfrreasonsRepository.findByLssitemasterAndCommentsAndStatus(objClass.getLssitemaster(), objClass.getComments(), 1);
...
//		List<LSaudittrailconfiguration> lstAudit = LSaudittrailconfigurationRepository.findByLsusermasterOrderByOrdersequnce(LSaudittrailconfiguration);
		if(LSaudittrailconfiguration.getLssitemaster()!=null) {
			
		
		List<LSaudittrailconfiguration> lstAudit = LSaudittrailconfigurationRepository.findByLssitemasterOrderByAuditcofigcodeDesc(LSaudittrailconfiguration.getLssitemaster());
		...
		if (lstAudit != null && lstAudit.size() > 0) {
			...
			maprAuditConfig.put("AuditConfig", new ArrayList<>());
			maprAuditConfig.put("AuditConfigmaster", new ArrayList<>());
		}
	...
	public Map<String, Object> GetAuditconfig(Map<String, Object> argObj) {
		...
		final Integer sitecode = mapper.convertValue(obj1.get("sitecode"), Integer.class);

		if(sitecode != null) {
			LSSiteMaster siteobj = new LSSiteMaster();
			siteobj.setSitecode(sitecode);
			
			List<LSaudittrailconfiguration> auditconfig = LSaudittrailconfigurationRepository.findByLssitemasterOrderByAuditcofigcodeDesc(siteobj);
	...
	public List<LSaudittrailconfiguration> SaveAuditconfigUser(LSaudittrailconfiguration[] lsAuditary) {
		...
		LSaudittrailconfigurationRepository.saveAll(lsAudit);
//		lscfttransactionRepository.save(lsAudit.get(0).getObjsilentauditlst());
	...
	public List<LSreviewdetails> ReviewBtnValidation(LSreviewdetails[] objreview1) {
		...
		LSreviewdetailsRepository.saveAll(objreview);
	...
	public Map<String, Object> GetReviewDetails12(LSreviewdetails[] objreview1) {
		...
		List<LScfttransaction> cfttDeatils = lscfttransactionRepository
		        .findByserialnoIn(Collections.singletonList(objreviewdetails.get(0).getSerialno()));
...
//                })
//                .collect(Collectors.toList());
		LscfrtransactiononorderRepository.saveAll(obj);
		cfttransaction.setObjResponse(new Response());
		cfttransaction.getObjResponse().setStatus(true);
		cfttransaction.getObjResponse().setInformation("");
	...
	public List<LScfttransaction> silentandmanualRecordHandlerlist(LScfttransaction[] mapObj) {
		...
		lscfttransactionRepository.saveAll(obj);
		return obj;
	}
	...
	public LScfttransaction beforloginsilentandmanualRecordHandler(Map<String, Object> mapObj) {
