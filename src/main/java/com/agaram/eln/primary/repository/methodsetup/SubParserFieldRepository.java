...
//	List<SubParserField> getSubParserFieldByMethodKey(final int methodKey);
	...
	@Transactional
	@Modifying
	@Query(value = "SELECT SubParserField.* FROM SubParserField " +
	        "JOIN ParserField ON ParserField.parserfieldkey = SubParserField.parserfieldkey " +
	        ...
	        "WHERE Method.methodkey = ?1 AND SubParserField.status = 1",
	        nativeQuery = true)
	List<SubParserField> getSubParserFieldByMethodKey(final int methodKey);
