...
//			   returnObject.put("PDInstFieldDataMap", outputMap);  	
	 ...
	 */
	   ...
	   @SuppressWarnings("unchecked")
	   @Transactional
	   public ResponseEntity<Object> evaluateParserMultiple(final int methodKey,final LSSiteMaster site,final List<String> rawDataContents, final String tenant,final Integer isMultitenant,
	       ...
	       if (methodByKey.isPresent()) {
	           ...
	           // Store parsed data for all files
	           Map<String, Object> bulkParsedData = new HashMap<>();

	           int fileIndex = 1;
	           for (String rawDataContent : rawDataContents) {
	               Map<String, Object> parserObjectMap =
	                       (Map<String, Object>) getMethodFieldList(methodKey, site, rawDataContent, tenant, isMultitenant).getBody();

	               Map<String, List<List<MethodFieldTechnique>>> parsedData =
	                       (Map<String, List<List<MethodFieldTechnique>>>) parserObjectMap.get("PDInstFieldDataMap");

	               // Keep parsedData grouped per file
	               bulkParsedData.put("file_" + fileIndex, parsedData);
	               fileIndex++;
	           }

	           return new ResponseEntity<>(bulkParsedData, HttpStatus.OK);
	       } else {
	           return new ResponseEntity<>("Method Not Found", HttpStatus.NOT_FOUND);
	       }
	   }
...
//	}
	...
	public Map<String, Object> insertELNResultDetails(List<ELNResultDetails> resultdetails , List<Integer> selectedProductsList) throws ParseException {
		...
		{
		resultfieldvaluesrepo.saveAll(resultdetails.get(i).getLsresultfieldvalues());
		}
		
		elnResultRepo.saveAll(resultdetails);
	...
	public Map<String, Object> onImportfornonreg(List<ELNResultDetails> resultdetails , List<Integer> selectedProductsList) throws ParseException {
		Map<String, Object> maps = new HashMap<String, Object>();
	     ...
	     List<RCTCPResultDetails> rctcpresultlist = new ArrayList<>();
			 ...
			 RCTCPResultDetailsRepository.saveAll(rctcpresultlist);
