...
//	List<ParserBlock> getParserBlockByMethodKey(final int methodKey);
	...
	@Query(value = "SELECT ParserBlock.* FROM ParserBlock JOIN Method " +
	        "ON Method.methodkey = ParserBlock.methodkey " +
	        "WHERE Method.methodkey = ?1 AND ParserBlock.status = 1", nativeQuery = true)
	List<ParserBlock> getParserBlockByMethodKey(final int methodKey);
