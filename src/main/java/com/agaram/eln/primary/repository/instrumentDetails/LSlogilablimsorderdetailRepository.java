...
public interface LSlogilablimsorderdetailRepository extends JpaRepository<LSlogilablimsorderdetail, Long> {
	...
	@Transactional
	@Modifying
	@Query(value = "update LSlogilablimsorderdetail  set sequenceid =?1, batchid = ?2, repeat=?3, autoregistercount=?4 ,lssamplefile_filesamplecode=?6 where batchcode = ?5",nativeQuery = true)
	void sevaluesBybatchcode(String seq,String batchid,Boolean repeat,Integer autoregistercount, Long batchcode, Integer lSsamplefile);

	@Transactional
	@Modifying
	@Query(value = "UPDATE LSlogilablimsorderdetail SET instrumentcode = ?1, methodcode = ?2, filecode = ?3, lsfile_filecode = ?3, lssamplefile_filesamplecode = ?4  WHERE batchcode = ?5 ", nativeQuery = true)
	void updateOrderFields(String instrumentcode, String methodcode, Integer filecode, Integer lssamplefilecode, Long batchcode);
	
	public List<LSlogilablimsorderdetail> findByFiletypeAndOrderflagOrderByBatchcodeDesc(Integer filetype,
			String orderflag);
	
	public List<LSlogilablimsorderdetail> findByRepeatAndAutoregistercountGreaterThan(boolean repeat,
			int count);
	...
	@Modifying
	@Query("UPDATE LSlogilablimsorderdetail l SET l.lsfile.filecode = :filecode WHERE l.batchcode = :batchcode")
	void updateFileRelationship(@Param("batchcode") Long batchcode, @Param("filecode") Integer filecode);

	@Modifying
	@Query("UPDATE LSlogilablimsorderdetail l SET l.lsworkflow.workflowcode = :workflowcode WHERE l.batchcode = :batchcode")
	void updateWorkflowRelationship(@Param("batchcode") Long batchcode, @Param("workflowcode") Integer workflowcode);

	@Modifying
	@Query("UPDATE LSlogilablimsorderdetail l SET l.lsautoregisterorders.regcode = :regcode WHERE l.batchcode = :batchcode")
	void updateAutoRegisterRelationship(@Param("batchcode") Long batchcode, @Param("regcode") Long regcode);
...
//	@Query(value = "SELECT o.batchcode,o.batchid,o.approved,o.approvelstatus, CAST((SELECT filenameuser FROM lsfile WHERE filecode = o.lsfile_filecode) AS varchar(10)) AS filename, testname, CAST((SELECT projectname FROM LSprojectmaster WHERE projectcode = o.lsprojectmaster_projectcode) AS varchar(10)) AS projectname, CAST((SELECT samplename FROM LSsamplemaster WHERE samplecode = o.lssamplemaster_samplecode) AS varchar(10)) AS samplename, o.createdtimestamp, o.keyword  FROM LSlogilablimsorderdetail o " +
	@Query(value = "SELECT * FROM LSlogilablimsorderdetail o INNER JOIN  lsfile l on l.filecode = o.lsfile_filecode "
			...
			+ "AND (LOWER(o.batchid) LIKE LOWER(?1) OR LOWER(o.sequenceid) LIKE LOWER(?1) OR LOWER(o.testname) LIKE LOWER(?1) OR LOWER(o.keyword) LIKE LOWER(?1) OR LOWER(l.filenameuser) LIKE LOWER(?1))) "
			+ "OR (o.filetype = 0 AND (LOWER(o.batchid) LIKE LOWER(?1) OR LOWER(o.sequenceid) LIKE LOWER(?1) OR LOWER(o.testname) LIKE LOWER(?1) OR LOWER(o.keyword) LIKE LOWER(?1) OR LOWER(l.filenameuser) LIKE LOWER(?1))) "
			...
			+ "OR (o.viewoption = 3 " + "AND o.lsusermaster_usercode IN (?3) And o.teamselected = false "
			+ "AND o.lsprojectmaster_projectcode IS NULL "
			+ "AND (LOWER(o.batchid) LIKE LOWER(?1) OR LOWER(o.sequenceid) LIKE LOWER(?1) OR LOWER(o.testname) LIKE LOWER(?1) OR LOWER(o.keyword) LIKE LOWER(?1) OR LOWER(l.filenameuser) LIKE LOWER(?1)))"
			
			+ "OR (o.viewoption = 3 " 
			+ "AND o.teamselected = true "
			+ "AND o.batchcode IN ?7 " 
			+ "AND o.lsprojectmaster_projectcode IS NULL " 
			+ "AND (LOWER(o.batchid) LIKE LOWER(?1) " 
			+ "OR LOWER(o.testname) LIKE LOWER(?1) " 
			+ "OR LOWER(o.keyword) LIKE LOWER(?1) " 
			+ "OR LOWER(l.filenameuser) LIKE LOWER(?1)))"
	...
	public List<LSlogilablimsorderdetail> getLSlogilablimsorderdetailsearchrecords(String searchkey, Integer userCode,
			List<LSuserMaster> userCodes, Integer sitecode, int i, Integer pageperorder,List<Long> selectedteambatchCodeList);

	@Transactional
	@Query(value = "SELECT count(*) FROM LSlogilablimsorderdetail o INNER JOIN  lsfile l on l.filecode = o.lsfile_filecode "
			+ "WHERE (o.lsprojectmaster_projectcode IN (SELECT lsprojectmaster_projectcode FROM LSlogilablimsorderdetail WHERE lsprojectmaster_projectcode IN (SELECT projectcode FROM LSprojectmaster WHERE lsusersteam_teamcode IN (SELECT teamcode FROM LSuserteammapping WHERE lsuserMaster_usercode = ?2) AND status = 1)) "
			+ "AND (LOWER(o.batchid) LIKE LOWER(?1) OR LOWER(o.testname) LIKE LOWER(?1) OR LOWER(o.keyword) LIKE LOWER(?1) OR LOWER(l.filenameuser) LIKE LOWER(?1))) "
			+ "OR (o.filetype = 0 AND (LOWER(o.batchid) LIKE LOWER(?1) OR LOWER(o.testname) LIKE LOWER(?1) OR LOWER(o.keyword) LIKE LOWER(?1) OR LOWER(l.filenameuser) LIKE LOWER(?1))) "
			+ "OR (o.lsprojectmaster_projectcode IS NULL "
			+ "AND o.lssamplemaster_samplecode IN (SELECT DISTINCT m.samplecode FROM LSsamplemaster m JOIN lslogilablimsorderdetail d ON m.samplecode = d.lssamplemaster_samplecode WHERE m.lssitemaster_sitecode = ?4 AND m.status = 1) "
			+ "AND (LOWER(o.batchid) LIKE LOWER(?1) OR LOWER(o.testname) LIKE LOWER(?1) OR LOWER(o.keyword) LIKE LOWER(?1) OR LOWER(l.filenameuser) LIKE LOWER(?1))) "
			+ "OR (o.lsprojectmaster_projectcode IS NULL " + "AND o.viewoption = 1 "
			...
			+ "AND o.lsprojectmaster_projectcode IS NULL "
			+ "AND (LOWER(o.batchid) LIKE LOWER(?1) OR LOWER(o.testname) LIKE LOWER(?1) OR LOWER(o.keyword) LIKE LOWER(?1) OR LOWER(l.filenameuser) LIKE LOWER(?1)))", nativeQuery = true)
...
//			LSSiteMaster lssitemaster, int m, Integer pageperorder,Integer approvelstatus);
	...
	@Query(value = "WITH TeamProjects AS ( " +
	        ...
	        "       AND status = 1) AND sitecode =?15" +
	        ") " +
	        
	        
	        "SELECT * FROM LSlogilablimsorderdetail o " +
	        "WHERE o.orderflag = ?1 AND createdtimestamp BETWEEN ?3 AND ?4 AND sitecode =?15 " +
	        "AND (  " +
	        "   (filetype = ?2 AND  (o.Approvelstatus != ?13 OR o.Approvelstatus IS NULL) AND sitecode =?15 ) " +
	        "   OR (lsprojectmaster_projectcode IS NULL AND ordercancell IS NULL AND assignedto_usercode IS NULL AND sitecode =?15 AND (o.Approvelstatus != ?13 OR o.Approvelstatus IS NULL) " +
	        "       AND " +
				        "( " +
				    		"( " +
				    		"(viewoption = ?8) AND " +
				    		"( " +
				    		    "(teamselected =?16 AND batchcode IN ?14)" +
				    		") " +
				    		") " +
				    		"OR " +
				    		"( " +
				    		    ...
				    		    "(viewoption = ?8 AND teamselected =?17 AND lsusermaster_usercode IN ?9) " +
				    		") " +
				    ") " +
	        ") " +
	       
	        "   OR (lsusermaster_usercode = ?5 AND sitecode =?15 AND assignedto_usercode != ?5 AND assignedto_usercode IS NOT NULL AND (o.Approvelstatus != ?13 OR o.Approvelstatus IS NULL)) " +
	        "   OR (assignedto_usercode = ?5 AND sitecode =?15 AND  (o.Approvelstatus != ?13 OR o.Approvelstatus IS NULL)) " +
	        "   OR (o.lsprojectmaster_projectcode IN (SELECT lsprojectmaster_projectcode FROM TeamProjects) " +
	        "       AND ordercancell IS NULL AND sitecode =?15 AND assignedto_usercode IS NULL AND (o.Approvelstatus != ?13 OR o.Approvelstatus IS NULL)) " +
	        "   OR (o.lsprojectmaster_projectcode IN (SELECT lsprojectmaster_projectcode FROM TeamProjects) " +
	        "       AND lsusermaster_usercode = ?5 AND sitecode =?15 AND  ordercancell IS NULL AND (o.Approvelstatus != ?13 OR o.Approvelstatus IS NULL)) " +
	        "   OR (lsprojectmaster_projectcode IS NULL AND sitecode =?15 AND elnmaterial_nmaterialcode IN " +
	        ...
	        "       AND (o.Approvelstatus != ?12 OR o.Approvelstatus IS NULL) " +
	        " 		AND viewoption != ?6 AND viewoption != ?7 AND viewoption != ?8) " +
	        ") " +
	        "ORDER BY batchcode DESC " +
	        "OFFSET ?11 ROWS FETCH NEXT ?12 ROWS ONLY", nativeQuery = true)
	List<LSlogilablimsorderdetail> getLSlogilablimsorderdetaildashboardforpending(String string, int i, Date fromdate,
			Date todate, Integer integer2, int j, int k, int l,List<Integer> usercodelist,
			Integer integer3, int m, Integer pageperorder,Integer approvelstatus,List<Long> batchcode, Integer integer,Boolean teamtrue,Boolean teamfalse);
	...
	@Query(value = "WITH TeamProjects AS ( " +
			...
			"SELECT * FROM LSlogilablimsorderdetail o " +
					...
					"AND ( " +
					"   (filetype = ?2 AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL)) " +
					
					"OR (lsprojectmaster_projectcode IS NULL AND ordercancell IS NULL AND assignedto_usercode IS NULL AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL) " +
					"       AND " +
					"( " +
					
						"(viewoption = ?8 AND batchcode != -1) AND " +
						"( " +
						    "(teamselected = true AND batchcode IN ?11)" +
						") " +
					...
					"( " +
						    "(viewoption = ?6 AND lsusermaster_usercode = ?5) OR " +
						    "(viewoption = ?7 AND lsusermaster_usercode = ?5) OR " +
						    "(viewoption = ?8 AND teamselected = false AND lsusermaster_usercode IN ?9) " +
					") " +
					")) " +"ORDER BY batchcode DESC " , nativeQuery = true)

	
	List<LSlogilablimsorderdetail> getLSlogilablimsorderdetaildashboardforpendingview(String string, int i, Date fromdate,
			Date todate, LSuserMaster objuser, int j, int k, int l, List<LSuserMaster> usernotify,
			Integer approvelstatus,List<Long> selectedbatchcode);
	...
	@Query(value = "WITH TeamProjects AS ( " +
	        ...
	        "           FROM LSuserteammapping " +
	        "           WHERE lsuserMaster_usercode = ?5 AND teamcode IS NOT NULL) " +
	        "       AND status = 1 AND sitecode = ?12) " +
	        ") " +
	        "SELECT COUNT(*) FROM LSlogilablimsorderdetail o " +
	        "WHERE o.orderflag = ?1 AND createdtimestamp BETWEEN ?3 AND ?4 AND sitecode = ?12 " +
	        "AND ( " +
	        "   (filetype = ?2 AND sitecode = ?12 AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL)) " +
	        
			"   OR (lsprojectmaster_projectcode IS NULL AND sitecode = ?12 AND ordercancell IS NULL AND assignedto_usercode IS NULL AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL) " +
			"       AND (viewoption = ?8 AND teamselected =?13 AND batchcode IN ?11) " +
			"   ) " +
	        "   OR (lsprojectmaster_projectcode IS NULL AND sitecode = ?12 AND ordercancell IS NULL AND assignedto_usercode IS NULL AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL) " +
	        "       AND ((viewoption = ?6 AND lsusermaster_usercode = ?5) OR " +
	        "             (viewoption = ?7 AND lsusermaster_usercode = ?5) OR " +
	        "             (viewoption = ?8 AND teamselected =?14 AND lsusermaster_usercode IN (?9))) " +
	        "   ) " +
	       
	        "   OR (lsusermaster_usercode = ?5 AND sitecode = ?12 AND assignedto_usercode != ?5 AND assignedto_usercode IS NOT NULL AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL)) " +
	        "   OR (assignedto_usercode = ?5 AND sitecode = ?12 AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL)) " +
	        "   OR (o.lsprojectmaster_projectcode IN (SELECT lsprojectmaster_projectcode FROM TeamProjects) " +
	        "       AND ordercancell IS NULL AND sitecode = ?12 AND assignedto_usercode IS NULL AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL)) " +
	        "   OR (o.lsprojectmaster_projectcode IN (SELECT lsprojectmaster_projectcode FROM TeamProjects) " +
	        "       AND lsusermaster_usercode = ?5 AND sitecode = ?12 AND ordercancell IS NULL AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL)) " +
	        "   OR (lsprojectmaster_projectcode IS NULL AND sitecode = ?12 AND elnmaterial_nmaterialcode IN " +
	        "       (SELECT DISTINCT elnmaterial_nmaterialcode " +
	        "        FROM lslogilablimsorderdetail " +
	        "        WHERE elnmaterial_nmaterialcode IN " +
	        "              (SELECT m.nmaterialcode " +
	        "               FROM elnmaterial m " +
	        "               WHERE m.nsitecode = ?12)) " +
	        "       AND ordercancell IS NULL AND assignedto_usercode IS NULL AND lsusermaster_usercode != ?5 " +
	        "       AND (o.Approvelstatus != ?10 OR o.Approvelstatus IS NULL)"+
	        "       AND(viewoption != ?6 AND viewoption != ?7 AND viewoption != ?8)) " +
	        ") ", nativeQuery = true)
	long countLSlogilablimsorderdetaildashboardforpending(String string, int i, Date fromdate,
	        Date todate, Integer integer, int j, int k, int l, List<Integer> usercodelist,
	        Integer approvelstatus,List<Long> selectedbatchcodelist, Integer sitecode,Boolean teamtrue , Boolean teamfalse);
...
//	@Modifying
	@Query(value = "SELECT  count(*)  FROM LSlogilablimsorderdetail o "
			+ "WHERE (o.orderflag =?1 And lsprojectmaster_projectcode IS NULL And elnmaterial_nmaterialcode in (select DISTINCT elnmaterial_nmaterialcode from lslogilablimsorderdetail where elnmaterial_nmaterialcode in (select m.nmaterialcode from elnmaterial m where  m.nsitecode =?6))And createdtimestamp BETWEEN ?3 And ?4 And approvelstatus !=?2 And ordercancell IS NULL And assignedto_usercode IS NULL And lsusermaster_usercode !=?5 AND sitecode = ?6)"
			+ "OR(o.orderflag =?1 And lsprojectmaster_projectcode IS NULL And elnmaterial_nmaterialcode in (select DISTINCT elnmaterial_nmaterialcode from lslogilablimsorderdetail where elnmaterial_nmaterialcode in (select m.nmaterialcode from elnmaterial m where  m.nsitecode =?6))And createdtimestamp BETWEEN ?3 And ?4 And approvelstatus IS NULL And ordercancell IS NULL And assignedto_usercode IS NULL And lsusermaster_usercode !=?5 AND sitecode = ?6)", nativeQuery = true)
...
//	@Modifying
	@Query(value = "SELECT  count(*) FROM LSlogilablimsorderdetail o "
			+ "WHERE (approvelstatus =?1  And lsprojectmaster_projectcode IS NULL And elnmaterial_nmaterialcode in (select DISTINCT elnmaterial_nmaterialcode from lslogilablimsorderdetail where elnmaterial_nmaterialcode in (select m.nmaterialcode from elnmaterial m where  m.nsitecode =?5)) And createdtimestamp BETWEEN ?2 And ?3  And assignedto_usercode IS NULL And lsusermaster_usercode !=?4 AND sitecode = ?5)", nativeQuery = true)
	long getLSlogilablimsorderdetaildashboardforrejectcount(int i, Date fromdate, Date todate, Integer integer2,
			 Integer integer);
...
//	@Modifying
	...
	List<ordersinterface> findByOrderflagAndLsprojectmasterInAndLsworkflowInAndAssignedtoIsNullAndCreatedtimestampBetweenAndSitecodeOrAssignedtoAndCreatedtimestampBetweenAndSitecodeOrderByBatchcodeDesc(
			String string, List<LSprojectmaster> lstproject, List<LSworkflow> lstworkflow, Date fromdate, Date todate,
			Integer sitecode, LSuserMaster objuser, Date fromdate2, Date todate2, Integer sitecode2);


	List<Logilaborders> findByLsprojectmasterAndSitecodeAndCreatedtimestampBetweenAndDirectorycodeIsNullOrderByBatchcodeDesc(
			LSprojectmaster lsprojectmaster, Integer sitecode, Date fromdate, Date todate);

	List<Logilaborders> findByLsprojectmasterAndSitecodeAndCreatedtimestampBetweenAndDirectorycodeIsNullAndOrderflagOrderByBatchcodeDesc(
			LSprojectmaster lsprojectmaster, Integer sitecode, Date fromdate, Date todate, String orderflag);

	List<Logilaborders> findByLsprojectmasterAndSitecodeAndCreatedtimestampBetweenAndDirectorycodeIsNullAndFiletypeOrderByBatchcodeDesc(
			LSprojectmaster lsprojectmaster, Integer sitecode, Date fromdate, Date todate, Integer filetype);

	List<Logilaborders> findByLsprojectmasterAndSitecodeAndCreatedtimestampBetweenAndDirectorycodeIsNullAndOrderflagAndApprovelstatusAndFiletypeOrderByBatchcodeDesc(
			LSprojectmaster lsprojectmaster, Integer sitecode, Date fromdate, Date todate, String orderflag,
			Integer approvelstatus, Integer filetype);

	List<Logilaborders> findByLsprojectmasterAndSitecodeAndCreatedtimestampBetweenAndDirectorycodeIsNullAndOrderflagAndFiletypeOrderByBatchcodeDesc(
			LSprojectmaster lsprojectmaster, Integer sitecode, Date fromdate, Date todate, String orderflag,
			Integer filetype);

	@Transactional
	@Modifying
	@Query(value ="UPDATE LSlogilablimsorderdetail  SET lsworkflow_workflowcode =?3 WHERE sitecode = ?1 AND batchcode IN (?2)", nativeQuery = true)
	void setWorkflownullfororders(Integer integer, List<Long> ordercode, int workflowcode);

	long countBySitecodeAndLsworkflowIsNullAndApprovedIsNullAndOrderflagAndOrdercancellIsNull(Integer sitecode,
			String string);

	List<LSlogilablimsorderdetail> findBySitecodeAndLsworkflowIsNullAndApprovedIsNullAndOrderflagAndOrdercancellIsNull(
			Integer sitecode, String string);



	List<Logilaborderssh> findByOrderflagAndBatchcodeInOrderByBatchcodeDesc(String orderflag, List<Long> batchcode);

	List<Logilaborderssh> findByOrderflagAndBatchcodeInAndFiletype(String orderflag, List<Long> batchcode,
			Integer filetype);

	long countByLsworkflowAndOrderflagAndApprovedIsNullAndOrdercancellIsNull(LSworkflow objflow, String string);

	List<LSlogilablimsorderdetail> findByBatchcodeInAndOrderflag(List<Long> batchcode, String string);

	List<LSlogilablimsorderdetail> findByOrderflagAndLsprojectmasterAndOrdercancellIsNull(String string,
			LSprojectmaster objClass);

	List<LogilabOrdermastersh> findByOrderflagAndLsprojectmasterInAndAssignedtoIsNullAndLockeduserIsNotNullOrOrderflagAndLsprojectmasterIsNullAndAssignedtoIsNullAndLockeduserIsNotNull(
			String string, List<LSprojectmaster> lstproject, String string2);

	LSlogilablimsorderdetail findByBatchcodeAndCreatedtimestampBetweenOrderByBatchcodeDesc(Long batchcode,
			Date fromDate, Date toDate);

	LSlogilablimsorderdetail findTopByOrderByBatchcodeDesc();
	@Transactional
	@Modifying
	@Query("UPDATE LSlogilablimsorderdetail  SET repeat = ?1 WHERE batchcode = ?2 ")
