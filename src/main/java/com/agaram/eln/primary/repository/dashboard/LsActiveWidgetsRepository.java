...
public interface LsActiveWidgetsRepository extends JpaRepository<LsActiveWidgets, Integer>{
    ...
    @Query(value = "WITH CTE AS ("
            ...
            + "WHERE activewidgetscode IN (SELECT activewidgetscode FROM CTE) AND userId = :userId AND sitecode = :sitecode", 
            nativeQuery = true)
    void deleteCustomRows(@Param("offset") int offset, @Param("userId") Integer userId,@Param("sitecode") Integer integer);
	...
	List<LsActiveWidgets> findFirst50ByUserIdAndSitecodeOrderByActivewidgetscodeDesc(Integer usercode,
			Integer sitecode);
