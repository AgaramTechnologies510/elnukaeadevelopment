...
//		final List<Method> methodList =  methodRepo.findBySiteAndStatus(site, 1,
		...
		final List<Method> methodList =  methodRepo.findBySite(site,
				Sort.by(Sort.Direction.DESC, "methodkey"));
...
//		final InstrumentMaster instMaster = instMastRepo.findOne(methodMaster.getInstmaster().getInstmastkey());
		...
		final LSuserMaster createdUser = getCreatedUserByKey(methodMaster.getCreatedby().getUsercode());
		final Equipment equipment = equipmentrepository.findById(methodMaster.getEquipment().getNequipmentcode()).orElse(null);
		...
		if (equipment != null) {			
			...
			{							
				...
				methodversionrepository.saveAll(methodMaster.getMethodversion());
...
//}	
	...
	{	  		
		boolean saveAuditTrail=true;	
		//final InstrumentMaster instMaster = instMastRepo.findOne(method.getInstmaster().getInstmastkey());
		 final Equipment equipment = equipmentrepository.findById(method.getEquipment().getNequipmentcode()).orElse(null);
		 final Optional<Method> methodByKey = methodRepo.findByMethodkeyAndStatusAndSite(method.getMethodkey(), 1,site);
...
//							
   ...
   public ResponseEntity<Object> getInstListToAssociateMethod(final LSSiteMaster site){
	...
	{
		final LSuserMaster createdBy =  userRepo.findById(userKey).orElse(null);
	   ...
	   try
        {			
		  ...
		  // final String ext = FilenameUtils.getExtension(fileName); 
		   String rawDataText="";
		   CloudParserFile obj =null;
		   	   
		  // obj =  (blobid != null && !blobid.equals("")) ? cloudparserfilerepository.findByFileid(blobid) : cloudparserfilerepository.findTop1Byfilename(fileName);
		  obj = cloudparserfilerepository.findTop1Byfilename(fileName);
		  
		   if(obj!=null) {
				   String fileid = obj.fileid;
				   String ext =null;
				   
				   ext = obj.getExtension().contains(".") ?obj.getExtension().substring(obj.getExtension().lastIndexOf(".") + 1) : obj.getExtension();
...
//		final String ext = FilenameUtils.getExtension(fileName);
		final String ext = fileName != null && fileName.contains(".") ? fileName.substring(fileName.lastIndexOf('.') + 1)  : fileName;
...
//			File originalfiletemploc = stream2file(resource.getInputStream(), fileName, "."+ext);
			byte[] fileBytes = IOUtils.toByteArray(resource.getInputStream()); // from Apache Commons IO
			File originalfiletemploc = stream2file(new ByteArrayInputStream(fileBytes), fileName, "." + ext);
...
//		            Document convertPDFDocumentToText = new Document(resource.getInputStream());
	 		        Document convertPDFDocumentToText = new Document(new ByteArrayInputStream(fileBytes));
		            TextAbsorber textAbsorber = new TextAbsorber(new TextExtractionOptions(TextExtractionOptions.TextFormattingMode.Pure));
	 	          ...
	 	          rawDataText = new String(bytesArray, StandardCharsets.UTF_8);
 					...
 					InputStream licenseStream = getLicenseStream();
	 				  ...
	 				  final File convertedcsvfile = File.createTempFile(fileName, "csv");
	 				     ...
	 				     //   Document convertPDFDocumentToCSV = new Document(resource.getInputStream());
	 		 		        Document convertPDFDocumentToCSV = new Document(new ByteArrayInputStream(fileBytes));
...
//				            }
					      ...
					      try (FileInputStream fis = new FileInputStream(originalfiletemploc);
						    BufferedWriter writer = new BufferedWriter(new FileWriter(tempconvertedFile))) {
...
//						.collect(Collectors.joining("\n"));
				...
				// Use another fresh stream for reading raw text
				rawDataText = new BufferedReader(
   ...
   public Method findByMethodkey(final int methodKey) {
	   List<Method> method = methodRepo.findByMethodkey(methodKey);
	   return (method.get(0));
   }
...
//	   final int instrumentKey = (Integer) mapObject.get("instMasterKey");
	   ...
	   final Optional<Method> methodByKey = methodRepo.findByMethodkeyAndStatus(methodKey, 1);	   
	   final Equipment equipment = equipmentrepository.findById(nequipmentcode).orElse(null);
	   final LSuserMaster createdUser = getCreatedUserByKey(doneByUserKey);
...
//			final List<CustomField> savedCustomFieldList =  
					customFieldRepo.saveAll(customFieldListBS);					
