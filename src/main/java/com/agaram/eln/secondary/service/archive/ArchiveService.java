...
public class ArchiveService {
	...
	public List<LScfttransaction> CreateArchive(LScfrArchiveHistory objcfrArchiveHistory,List<Integer> archivecode)
	{
		...
		List<LScfrachivetransaction> lsarchive = lstcfrtransaction.stream()
		        .map(lscfrtransaction -> new LScfrachivetransaction(null,lscfrtransaction.getModuleName(),lscfrtransaction.getReason(),
		...
		lsarchive =lsarchive.stream().map((items) ->{
			items.setSerialno(null);
			return items;
		}).collect(Collectors.toList());
		lscfrachivetransactionRepository.saveAll(lsarchive);
