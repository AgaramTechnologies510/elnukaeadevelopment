 ...
 * to access the Delimters Service methods.
 * 
 * @author ATE153
 * @version 1.0.0
 * @since 07- Feb- 2020
...
//		return delimitersService.getActiveDelimiters(sortBy);
		...
		if (obj == null) {
			...
			// final LSSiteMaster site = mapper.convertValue(mapObject.get("sitecode"),
			// LSSiteMaster.class);
			...
			// final String sortBy = mapper.convertValue(mapObject.get("sortOrder"),
			// String.class);
	 ...
	 * @param mapObject [Map] object with keys 'delimiters'- [Delimiters] Entity
	 *                  with 'delimitername'(m), 'actualdelimiter'(m),
	...
	public ResponseEntity<Object> createDelimiters(final HttpServletRequest request,
...
//		final Boolean saveAuditTrail = mapper.convertValue(mapObject.get("auditInput"), Boolean.class);
	...
	public ResponseEntity<Object> updateDelimiters(final HttpServletRequest request,
			@Validated @RequestBody Map<String, Object> mapObject) throws Exception {
		final ObjectMapper mapper = new ObjectMapper();
		final Delimiter delimiters = mapper.convertValue(mapObject.get("delimiters"), Delimiter.class);
...
//		  final int doneByUserKey = (Integer) mapObject.get("doneByUserKey");
	...
	public ResponseEntity<Object> deleteDelimiters(final HttpServletRequest request,
			@Validated @RequestBody Map<String, Object> mapObject) throws Exception {
		final ObjectMapper mapper = new ObjectMapper();
...
//		  final int doneByUserKey = (Integer) mapObject.get("doneByUserKey");
		...
		String strUserKey = (String) mapObject.get("doneByUserKey");
		final int doneByUserKey = Integer.parseInt(strUserKey);
		// final Delimiter delimiters = mapper.convertValue(mapObject.get("delimiters"),
		// Delimiter.class);
		final Delimiter otherdetails = mapper.convertValue(mapObject.get("otherdetails"), Delimiter.class);
		final Delimiter auditdetails = mapper.convertValue(mapObject.get("auditdetails"), Delimiter.class);

		return delimitersService.deleteDelimters(delimiterKey, site, comments, doneByUserKey, request, otherdetails,
				auditdetails);
	}
}
