...
//		final List<GeneralField> generalFieldList = generalFieldRepo.findBySiteAndStatus(site, 1,new Sort.Order(Sort.Direction.DESC, "generalfieldkey"));
		Sort sort = Sort.by(Sort.Direction.DESC, "generalfieldkey");
		List<GeneralField> generalFieldList = generalFieldRepo.findBySiteAndStatus(site, 1, sort);
	...
	{ 
	   final AppMaster appMaster = appMasterRepo.findById(generalField.getAppmaster().getAppmasterkey()).orElse(null);
