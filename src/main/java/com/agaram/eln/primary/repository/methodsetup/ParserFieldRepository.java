...
//			+ " ON m.methodkey = pb.method.methodkey AND m.methodkey = ?1 and pf.status=1" , nativeQuery = true)
	...
	@Query(value = "SELECT ParserField.* FROM ParserField " +
            "JOIN ParserBlock ON ParserField.parserblockkey = ParserBlock.parserblockkey " +
            "JOIN Method ON method.methodkey = ParserBlock.methodkey " +
            "WHERE method.methodkey = ?1 AND ParserField.status = 1", nativeQuery = true)
	
    List<ParserField> getParserFieldByMethodKey(Integer methodkey);

	List<ParserField> findByParserblockInAndStatusOrderByParserfieldkeyAsc(List<ParserBlock> lstparserblock, int i);
