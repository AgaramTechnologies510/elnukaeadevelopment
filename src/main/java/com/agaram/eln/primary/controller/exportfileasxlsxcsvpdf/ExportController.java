...
import com.agaram.eln.primary.view.CsvView;
import com.agaram.eln.primary.view.ExcelView;
...
public class ExportController {
    ...
    public ModelAndView exportFile( @RequestBody Map<String, Object> mapObject)throws Exception {
	    ...
	    String exportFormat =
	            mapper.convertValue(mapObject.get("exportFormat"), String.class);
	    
	    if ("pdf".equalsIgnoreCase(exportFormat)) {
	        mav.setView(new PdfView());     // ⭐ REQUIRED
	    } else if ("csv".equalsIgnoreCase(exportFormat)) {
	        mav.setView(new CsvView());
	    } else {
