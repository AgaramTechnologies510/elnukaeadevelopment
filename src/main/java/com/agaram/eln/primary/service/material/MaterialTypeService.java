...
public class MaterialTypeService {
	...
	public ResponseEntity<Object> getMaterialType(MaterialType objMaterialType) {
		List<MaterialType> lstmaterialtype = objMaterialType.getNsitecode().equals(0) 
				? materialTypeRepository.findByNmaterialtypecodeNotAndNsitecodeNotAndNdefaultstatusOrderByNmaterialtypecodeDesc(-1, -1, 3)
				: materialTypeRepository.findByNmaterialtypecodeNotAndNsitecodeAndNdefaultstatusOrderByNmaterialtypecodeDesc(
						-1,objMaterialType.getNsitecode(),3);
	...
	public ResponseEntity<Object> getMaterialTypeonId(MaterialType objMaterialType) {
		MaterialType lsmaterialtype = materialTypeRepository.findByNmaterialtypecode(objMaterialType.getNmaterialtypecode());
...
//				objMaterialType.setNmaterialtypecode(objlstTypes1.size()+1);
				...
				inventorybarcodemapRepository.saveAll(objMaterialType.getLstbarcodes());
	...
	public ResponseEntity<Object> syncSamplestoType() throws ParseException {
		...
		if(objPrefrence == null) {
			...
			if(!objMatLst.isEmpty()) {
				elnmaterialRepository.saveAll(objMatLst);
			}
			...
			if(!objMatSampleLst.isEmpty()) {
				...
				if(!objOrders.isEmpty()) {
					...
					lSlogilablimsorderdetailRepository.saveAll(objOrders);
				}
				...
				if(!objProOrders.isEmpty()) {
					...
					lSlogilabprotocoldetailRepository.saveAll(objProOrders);
				}
			}
