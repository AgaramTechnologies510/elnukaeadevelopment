...
public interface ElnmaterialRepository extends JpaRepository<Elnmaterial, Integer> {
	...
	Elnmaterial findByNmaterialcode(Integer materialcode);
	...
	@Query("SELECT e FROM Elnmaterial e WHERE e.nsitecode = :nsitecode "
	        ...
	        + "AND e.nmaterialcode IN (SELECT m.nmaterialcode FROM MaterialProjectMap m where e.nmaterialcode = m.nmaterialcode AND m.lsproject = :project) "
	...
	@Query("SELECT e FROM Elnmaterial e WHERE e.nsitecode = :nsitecode "
	        + "AND e.createddate BETWEEN :fromDate AND :toDate "
	        + "AND e.nmaterialcode NOT IN (SELECT m.nmaterialcode FROM MaterialProjectMap m where e.nmaterialcode = m.nmaterialcode) "
	        + "AND e.materialcategory.nmaterialcatcode IN "
	        + "(SELECT c.nmaterialcatcode FROM MaterialCategory c WHERE c.nstatus = 1) "
	        + "AND e.materialtype.nmaterialtypecode IN "
	        + "(SELECT t.nmaterialtypecode FROM MaterialType t WHERE t.nstatus = 1) "
	...
	List<Elnmaterial> findElnmaterialsWithoutProjectMapsforfilterforcat(
	        ...
	        @Param("toDate") Date toDate,
	        @Param("objMaterialCategory") MaterialCategory objMaterialCategory);
	Long countByNsitecode(Integer nsitecode);
	
	@Transactional
	@Query(value = "SELECT m.* FROM elnmaterial m INNER JOIN lslogilablimsorderdetail l ON l.elnmaterial_nmaterialcode = m.nmaterialcode WHERE m.nsitecode = ?1",
	       nativeQuery = true)
	List<Elnmaterial> getmaterialonsitecodebased(Integer sitecode);
