...
//  					 HttpStatus.CONFLICT);
		...
		{	
			...
			final InstrumentType instType = typeRepo.findByInsttypekey(master.getInsttype().getInsttypekey());
			master.setInsttype(instType);
	       		
	    	final InstrumentCategory instCategory = categoryRepo.findByInstcatkey(master.getInstcategory().getInstcatkey()); 
    ...
    private InstrumentRights saveAdminInstRights(final InstrumentMaster savedMaster) {
    	...
    	final LSuserMaster user =  userRepo.findByUsercode(1);
    		...
    		if (isEditable) {
    			...
    			if(instrumentByCode.isPresent())
				{
	     		...
	     		{   
	            	...
	            	final InstrumentType instType = typeRepo.findByInsttypekey(
	            				master.getInsttype().getInsttypekey());
	            	instrumentToSave.setInsttype(instType);
	            	
	            	final InstrumentCategory instCategory = categoryRepo.findByInstcatkey(
...
//	    			final InstrumentMaster instrumentBeforeSave = new InstrumentMaster(instrumentBykey.get());
    ...
    {        	    	
//    	final List<InstrumentMaster>  instobj = masterRepo.findBySite(site, new Sort.Order(Sort.Direction.DESC, "instmastkey"));
    	final List<InstrumentMaster> instobj = masterRepo.findBySite(
    		    site, 
    		    Sort.by(Sort.Direction.DESC, "instmastkey")
    		);
  ...
  //  @Transactional
    public ResponseEntity<Object> deleteInstMaster(final Integer instMastKey,
    	...
    	boolean saveAuditTrial1 = true;
      	...
      	final InstrumentMaster instMaster = masterRepo.findByInstmastkey(instMastKey);
