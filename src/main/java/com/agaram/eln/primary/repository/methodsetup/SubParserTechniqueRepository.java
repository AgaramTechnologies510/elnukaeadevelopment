...
//	List<SubParserTechnique> getSubParserTechniqueByMethodKey(final int methodKey);
	...
	@Transactional
	@Modifying
	@Query(value = "SELECT SubParserTechnique.* FROM SubParserTechnique " +
	        "JOIN ParserField ON ParserField.parserfieldkey = SubParserTechnique.parserfieldkey " +
	        "JOIN ParserBlock ON ParserBlock.parserblockkey = ParserField.parserblockkey " +
	        "JOIN Method ON Method.methodkey = ParserBlock.methodkey " +
	        "WHERE Method.methodkey = ?1 AND SubParserTechnique.status = 1",
	        nativeQuery = true)
	List<SubParserTechnique> getSubParserTechniqueByMethodKey(final int methodKey);
