...
/**
 * This abstract class holds methods that are used to create Pdf files
 * @author ATE128
 * @version 1.0.0
 * @since   07- Jan- 2020
 */
public abstract class AbstractPdfView extends AbstractView {
    ...
    protected final void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception  {
       ...
       // Document document = new Document(PageSize.A4.rotate(), 36, 36, 54, 36);
        ...
        Rectangle pageOrientation = PageSize.A4;     
        Rectangle rectangle = new Rectangle(30, 30, 500, 800);//(Left-30, Right-500, Bottom-30, top-800) -520*770
        if (model.get("pageOrientation") != null
        		 && ((String) model.get("pageOrientation")).equalsIgnoreCase("landscape"))
        {
        	pageOrientation = PageSize.A4.rotate();
        	rectangle = new Rectangle(30, 30, 800, 550);//(Left-30, Right-800, Bottom-30, top-550) -770*520
        }
        
        Document document = new Document(pageOrientation);
        ...
        buildPdfMetadata(model, writer, document, request);
     ...
     * @param request in case we need locale etc. Shouldn't look at attributes.
     */
    protected void buildPdfMetadata(Map<String, Object> model,PdfWriter writer, Document document, HttpServletRequest request) {
    }
