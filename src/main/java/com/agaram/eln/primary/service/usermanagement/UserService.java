...
public class UserService {
	...
	public LSusergroup InsertUpdateUserGroup(LSusergroup objusergroup) {
		...
		if (objusergroup.getUsergroupcode() != null) {
			LSusergroup objGroup = lSusergroupRepository.findById(objusergroup.getUsergroupcode()).orElse(null);
...
//	}
	...
	public List<LSuserMaster> GetUsers(LSuserMaster objusergroup) {
		...
		if (objusergroup.getUsername().equalsIgnoreCase("Administrator")
				|| objusergroup.getLsusergrouptrans().getUsergroupcode() == 1) {
...
//			return lsuserMasterRepository.findByUserretirestatusNotOrderByCreateddateDesc(1);
		...
		if (objusergroup.getObjResponse() != null
			...
			return lsuserMasterRepository.findByUsercodeInOrderByUsercodeDesc(usercode);
		} else {
			return lsuserMasterRepository.findByUsercodeInAndUserretirestatusNotOrderByUsercodeDesc(usercode, 1);
		}
...
//		List<LSuserMaster> kumu=lsuserMasterRepository.findByUsernameIgnoreCaseAndUsercodeIn(objusermaster.getUsername(), usercodes);
		if (objusermaster.getUsercode() == null && lsuserMasterRepository
			...
			LSuserMaster updateUser = lsuserMasterRepository.findById(objusermaster.getUsercode()).orElse(null);
...
//			objusermaster.setUserstatus(objusermaster.getUserstatus().equals("Active") || objusermaster.getUserstatus().equals("Locked") ? "A" : "D");
			...
			if (objusermaster.isReset()
					&& (updateUser.getPasswordstatus().equals(1) || updateUser.getPassword() == null)) {
				objusermaster.setResponse(new Response());
				objusermaster.getResponse().setStatus(false);
				objusermaster.getResponse().setInformation("IDS_MSG_USERPWD");
				return objusermaster;
			}
			...
			if (objusermaster.getMultiusergroupcode() != null && objusermaster.getUsercode() != null) {
				...
				if (objusermaster.getLsmultisites().size() > 0) {
					...
					List<LSMultisites> freshList = objusermaster.getLsmultisites().stream().map(site -> {
						LSMultisites newSite = new LSMultisites();
						newSite.setUsercode(site.getUsercode());
						newSite.setDefaultsiteMaster(site.getDefaultsiteMaster());
						newSite.setLssiteMaster(site.getLssiteMaster());
						return newSite;
					}).collect(Collectors.toList());

					LSMultisitesRepositery.saveAll(freshList);

					// LSMultisitesRepositery.saveAll(objusermaster.getLsmultisites());/** now*/
				}
				...
				LSMultiusergroupRepositery.saveAll(objects);
		...
		if (objusermaster.getLsmultisites().size() > 0) {
			LSMultisitesRepositery.saveAll(objusermaster.getLsmultisites());
		}
		if (objusermaster.getMultiusergroupcode() != null) {
			LSMultiusergroupRepositery.saveAll(objusermaster.getMultiusergroupcode());
		}
		...
		int deletedCountLSMultisites = LSMultisitesRepositery.deleteUsercodeIsNull();
		System.out.println("Deleted rows: " + deletedCountLSMultisites);
		int deletedCountLSMultiusergroup = LSMultiusergroupRepositery.deleteUsercodeIsNull();
		System.out.println("Deleted rows: " + deletedCountLSMultiusergroup);
...
//		LSuserMaster lsuserMasterObj = lsuserMasterRepository.findByusername(lscfttransactionObj.getLsuserMaster().getUsername());
	...
	public LSuserMaster ResetPassword(LSuserMaster objuser) {
		LSuserMaster updateUser = lsuserMasterRepository.findById(objuser.getUsercode()).orElse(null);
...
//			return lsusersteamRepository.findByLsuserteammappingInAndStatus(teams, 1);
	...
	public Map<String, Object> GetUserTeamonSitevise(LSSiteMaster objclass) {
		...
		map.put("user", lsuserMasterRepository.findByLssitemasterOrderByUsercodeDesc(objclass));
	...
	public Map<String, Object> GetUserRightsonGroup(LSusergroup lsusergroup) {
		...
		if (lsusergroup.getUsergroupcode() == null) {
			List<LSusergrouprightsmaster> lstUserrightsmaster = lsusergrouprightsmasterRepository
					.findAllByOrderBySequenceorderAscTasksequenceAsc();
			...
			List<LSusergrouprights> lstUserrights = lsusergrouprightsRepository
					.findByUsergroupidOrderBySequenceorderDescTasksequenceAsc(lsusergroup);
	...
	public Integer GetUserActiveStatus(Integer usercode) {

		LSuserMaster userobj = lsuserMasterRepository.findByusercode(usercode);
		Integer userstatus = null;
		if (userobj != null) {
			String status = userobj.getUserstatus();
			userstatus = status.equalsIgnoreCase("Deactive") ? 0 : 1;
		}
		return userstatus;
	}
	...
	public List<LSusergrouprights> SaveUserRights(LSusergrouprights[] lsrites) {
		...
		if (lsrights.get(0).getUsergroupid() == null) {
			...
			lsusergrouprightsRepository.deleteAll(lsusergrouprightsRepository
					.findByUsergroupidOrderBySequenceorderDescTasksequenceAsc(lsrights.get(0).getUsergroupid()));

			// Reset primary keys to avoid update/optimistic locking issues
			lsrights.forEach(r -> r.setOrderno(null));
			lsusergrouprightsRepository.saveAll(lsrights);
	...
	private void updateNotificationForUserRoleRights(List<LSusergrouprights> objRights) throws ParseException {
		...
		List<Integer> objnotifyuser = userobj.stream().filter(obj -> obj.getUsercode() != null) // Ensure both are not
																								// null
				.map(LSMultiusergroup::getUsercode).collect(Collectors.toList());
		;
	...
	public List<LSusersteam> GetLoggedUsersTeamSitewise(LSSiteMaster site, LSuserMaster userobj) {
		...
		List<LSusersteam> lstteam = lsusersteamRepository.findByLsuserteammappingInAndLssitemaster(lstteammap, site);
	...
	public LSuserMaster ValidateSignature(LoggedUser objuser) {
		...
		List<LSuserMaster> userobj = lsuserMasterRepository
				.findByUsernameIgnoreCaseAndUsercodeIn(objuser.getsUsername(), usercode);
...
//			
	...
	public Map<String, Object> Getnotification(LSnotification lsnotification) {
		Map<String, Object> objresmap = new HashMap<String, Object>();

		objresmap.put("newnotificationcount", lsnotificationRepository
				.countByNotifationtoAndIsnewnotificationAndNotificationforAndSitecodeOrNotifationtoAndIsnewnotificationAndNotificationforAndSitecode(
						lsnotification.getNotifationto(), 1, 1, lsnotification.getSitecode(),
						lsnotification.getNotifationto(), 1, 2, lsnotification.getSitecode()));

		objresmap.put("notification",
				lsnotificationRepository
						.findFirst20ByNotifationtoAndNotificationforAndSitecodeOrderByNotificationcodeDesc(
								lsnotification.getNotifationto(), lsnotification.getNotificationfor(),
								lsnotification.getSitecode()));

		objresmap.put("mynotificationcount",
				lsnotificationRepository.countByNotifationtoAndIsnewnotificationAndNotificationforAndSitecode(
						lsnotification.getNotifationto(), 1, 1, lsnotification.getSitecode()));
	...
	public Map<String, Object> Updatenotificationread(LSnotification lsnotification) {
		...
		objresmap = Getnotification(lsnotification);
	...
	public Map<String, Object> GetnotificationonLazyload(LSnotification lsnotification) {
		...
		objresmap.put("notification", lsnotificationRepository
				.findFirst20ByNotifationtoAndNotificationcodeLessThanAndNotificationforAndSitecodeOrderByNotificationcodeDesc(
						lsnotification.getNotifationto(), lsnotification.getNotificationcode(),
						lsnotification.getNotificationfor(), lsnotification.getSitecode()));
	...
	@SuppressWarnings("unused")
	public LSuserMaster Usersendpasswormail(LSuserMaster objusermaster)
		...
		if ((objusermaster.getIsmultitenant() == 2
			...
			// sso user mail microsoft / google
...
//				emailService.sendEmailelnLite(email);
	...
	public List<LSuserMaster> GetAllActiveUsers(LSuserMaster objuser) {
		if (objuser.getUnifieduserid() != null) {
			LSuserMaster Defaultuser = lsuserMasterRepository.findByUsercodeAndUnifieduseridIsNull(1);
			if (Defaultuser != null) {
				String unifieduser = Defaultuser.getUsername().toLowerCase().replaceAll("[^a-zA-Z0-9]", "") + "u"
						+ Defaultuser.getUsercode() + "s" + 1 + objuser.getUnifieduserid();
				Defaultuser.setUnifieduserid(unifieduser);
				lsuserMasterRepository.save(Defaultuser);
			}
		}
		List<Integer> lstuser = LSMultisitesRepositery.getusernameandusercode(objuser.getLssitemaster().getSitecode());
		if (lstuser == null) {
			lstuser = new ArrayList<>();
		}

//		lstuser.removeIf(usercode -> usercode == 1 || usercode.equals(objuser.getUsercode()));
		lstuser.removeIf(usercode -> usercode.equals(objuser.getUsercode()));

		return lsuserMasterRepository
				.findByUsercodeInAndUserstatusAndUserretirestatusAndUnifieduseridNotNullOrderByUsercodeDesc(lstuser,
						"A", 0);
	}
		...
		if (listofallmaster.getScreenname().equalsIgnoreCase("usermaster")) {
			...
			if (lsusermaster.size() == 0) {
				for (LSuserMaster usermansterobj : listofallmaster.getLsusermaster()) {
					...
					LSMultiusergroupRepositery.saveAll(usermansterobj.getMultiusergroupcode());
			...
			if (lstSection.isEmpty()) {
				...
				sectionRepository.saveAll(objLst);
...
//			listofallmaster.getObjResponse().setInformation("IDS_MSG_SUCCESSMSG");
			...
			elnmaterialRepository.saveAll(listofallmaster.getElnmaterial());
	...
	public Map<String, Object> GetSiteWiseUserGrouplist(LSuserMaster objclass) {
		...
		if (objclass.getUsername() != null) {
			for (LSSiteMaster tempobj : lsrights) {
				...
				if (isnewuser) {
					existingUsers = lsuserMasterRepository.findByUsernameIgnoreCaseAndUsercodeInAndUserretirestatusNot(
							objclass.getUsername(), usercode, 1);
				} else {
					existingUsers = lsuserMasterRepository
							.findByUsernameIgnoreCaseAndUsercodeInAndUsercodeNotAndUserstatusNotAndUserretirestatusNot(
...
//						break;
					...
					if (!isnewuser) {
						existingUsers = lsuserMasterRepository
								.findByUsernameIgnoreCaseAndUsercodeInAndUsercodeNotAndUserstatusNotAndUserretirestatusNotAndLoginfrom(
...
//		Integer siteCode = (Integer) mapObjects.get("sitecode");
...
//				});
//		List<LSusergroup> existingUserGroups = new ArrayList<>(
//			    lSusergroupRepository.findByLssitemasterAndUsergroupnameIgnoreCaseIn(siteCode, userGroupNameList)
//			        .stream()
//			        .collect(Collectors.toMap(
//			            LSusergroup::getUsergroupname,
//			            Function.identity(),         
...
////				item.setModifiedon(currentDate);s
//			}
//			return item;
//		}).collect(Collectors.toList());
//		lSusergroupRepository.saveAll(userGroupList); // kumu - user group Save
...
//				.findByusercodeIn(usercodelist).stream().map(LSMultisites::getUsercode)
//				.collect(Collectors.toList());
...
//		Map<String, List<LSuserMaster>> userMasterExistList = filteredUserMasterExistList.stream()
//		        .collect(Collectors.groupingBy(LSuserMaster::getUsername));
//		 Random random = new Random();
//		for (LSuserMaster item : userMasterList) {
//		    List<LSuserMaster> existUser = userMasterExistList.getOrDefault(item.getUsername(), new ArrayList<>());
//		    if (existUser.size() > 0) {
//		    	int uniqueNumber = 20 + random.nextInt(10000);
//		        item.setUsername(item.getUsername() + "-" + uniqueNumber);
//		    }
...
//		}
//		LSMultisitesRepositery.saveAll(multisiteList);
//		LSMultiusergroupRepositery.saveAll(multiuserGroupList);
//		lsuserMasterRepository.saveAll(userMasterList);// kumu - user Master Save
...
//		Map<String, LSusergroup> userGroupExistMap = existingUserGroups.stream()
//				.collect(Collectors.toMap(u -> u.getUsergroupname().toLowerCase(), Function.identity()));
		Map<String, LSusergroup> userGroupExistMap = existingUserGroups.stream()
			    .collect(Collectors.toMap(
			        u -> u.getUsergroupname().toLowerCase(),
			        Function.identity(),
			        (existing, replacement) -> existing // or replacement
			    ));
		userGroupList = userGroupList.stream().map(item -> {
			LSusergroup existing = userGroupExistMap.get(item.getUsergroupname().toLowerCase());
			if (existing != null) {
				return existing;
			} else {
				item.setCreatedon(currentDate);
		...
		try {
			...
			Map<String, LSusergroup> updatedUserGroupMap = userGroupList.stream()
					.collect(Collectors.toMap(u -> u.getUsergroupname().toLowerCase(), Function.identity()));
			List<LSuserMaster> existingUsers = lsuserMasterRepository.findByusernameIgnoreCaseIn(userNameList);
//			Map<String, LSuserMaster> existingUserMap = existingUsers.stream()
//					.collect(Collectors.toMap(u -> u.getUsername().toLowerCase(), Function.identity()));
			Map<String, LSuserMaster> existingUserMap = existingUsers.stream()
				    .collect(Collectors.toMap(
			...
			List<LSMultiusergroup> multiuserGroupList = new ArrayList<>();
			List<LSMultisites> multisiteList = new ArrayList<>();
			List<LSuserMaster> newUsersToInsert = new ArrayList<>();
			List<LSuserMaster> existingUsersToUpdate = new ArrayList<>();
			for (LSuserMaster user : userMasterList) {
				LSuserMaster existing = existingUserMap.get(user.getUsername().toLowerCase());

				if (existing != null) {
					LSusergroup targetGroup = updatedUserGroupMap.get(user.getUsergroupname().toLowerCase());

					if (targetGroup != null) {
						List<LSMultiusergroup> userGroups = Optional.ofNullable(existing.getMultiusergroupcode())
								.orElse(new ArrayList<>());

						boolean alreadyInGroup = userGroups.stream().anyMatch(
								mg -> mg.getLsusergroup().getUsergroupcode().equals(targetGroup.getUsergroupcode()));

						if (!alreadyInGroup) {
							LSMultiusergroup newGroup = new LSMultiusergroup();
							newGroup.setLsusergroup(targetGroup);
							newGroup.setDefaultusergroup(
									userGroups.stream().anyMatch(mg -> mg.getDefaultusergroup()!=null && mg.getDefaultusergroup() == 1) ? null : 1);
							userGroups.add(newGroup);
							multiuserGroupList.add(newGroup);
						}
						existing.setMultiusergroupcode(userGroups);
					}

					List<LSMultisites> userSites = Optional.ofNullable(existing.getLsmultisites())
							.orElse(new ArrayList<>());
					for (LSMultisites newSite : user.getLsmultisites()) {
						boolean exists = userSites.stream().anyMatch(
								s -> s.getLssiteMaster().getSitecode().equals(newSite.getLssiteMaster().getSitecode()));
						if (!exists)
							userSites.add(newSite);
					}
					existing.setLsmultisites(userSites);
					multisiteList.addAll(userSites);
					existingUsersToUpdate.add(existing);

				} else {
					user.setCreateddate(currentDate);
					user.setUnifieduserid(createUnifiedUserId(user, siteCode));
					...
					if (targetGroup != null) {
						LSMultiusergroup newGroup = new LSMultiusergroup();
						newGroup.setLsusergroup(targetGroup);
						newGroup.setDefaultusergroup(1);
						user.setMultiusergroupcode(Collections.singletonList(newGroup));
						multiuserGroupList.add(newGroup);
					}

					multisiteList.addAll(user.getLsmultisites());
			...
			if (!newUsersToInsert.isEmpty()) {
				lsuserMasterRepository.saveAll(newUsersToInsert);
			}
			if(!existingUsersToUpdate.isEmpty()) {
				newUsersToInsert.addAll(existingUsersToUpdate);
			}
			
			return new ResponseEntity<>(newUsersToInsert, HttpStatus.OK);
		}catch (Exception e) {
	...
	public List<LSuserMaster> sendEmailListWithBatch(Map<String, Object> objusermasterlist)
		...
		for (LSuserMaster objusermaster : objusermasterlistobj) {
			if (objusermaster.getPassword() != null) {
				...
				email.setMailto(objusermaster.getEmailid());
				email.setSubject("ELN User Credentials");
				String existingpassword = AESEncryption.decrypt(objusermaster.getPassword());
				email.setMailcontent(generateEmailContent(objusermaster, existingpassword));
				emaillist.add(email);
			}
		}
		if (emaillist.size() > 0) {
	...
	public List<LSuserMaster> getDesignation(Map<String, Object> obj) {
		LSSiteMaster objSite = new LSSiteMaster();
		String userCodeString = (String) obj.get("usercode");
		String sitecode = (String) obj.get("sitecode");
		String userName = (String) obj.get("username");
		String sitename = (String) obj.get("sitename");
		objSite.setSitecode(Integer.parseInt(sitecode));
		objSite.setSitename(sitename);
		return lsuserMasterRepository.findByUsercodeAndUsernameAndLssitemaster(Integer.parseInt(userCodeString),
				userName, objSite);
	}
}